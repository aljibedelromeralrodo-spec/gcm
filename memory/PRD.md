...
## 2026-08-31 — Lote grande (testeado 100% iter_68) + fixes UI ficha cliente
- SANITIZADOR PDF (pdf_service.sanitizar_gastos_operacionales): elimina páginas con Gastos/Costos Operacionales y VALIDA el resultado; si sigue contaminado BLOQUEA (409 en correo aprobación; excluido en flujos de guardado). Integrado en 3 flujos.
- QUITAR ADJUNTOS de correos en espera: POST /api/correos-preview/{pid}/adjunto/{idx}/quitar + botón ✕ en panel. Probado E2E con correo real de Claudia Zurita (quedó solo carta original + simulador limpio, SIN enviar).
- OVERLAY "Muéstrame en pantalla" (PantallaMartin.js): comando por texto/voz en el chat abre visor fullscreen (carpeta en grilla / documento con zoom / correos con ENVÍALO). Voz interna: más grande/chico, siguiente, cierra, envíalo. FIX clave: stopSpeaking antes del dispatch (los POST TTS del resumen saturaban las 6 conexiones HTTP y colgaban el GET). rxAcentos para búsqueda insensible a tildes.
- FORZAR CARPETA V3: variantes de nombre (todas / 1er nombre+apellidos / apellidos), búsqueda también en NOMBRES DE ADJUNTOS, conteo de menciones ("detecté N correos"), flag por_enriquecer. FIX: match de carpeta existente también con variantes (evita duplicados tipo "ANTONIA FERNANDA PEREZ MUÑOZ" vs "ANTONIA PÉREZ"). Fusioné el duplicado: ANTONIA PÉREZ ahora 32 docs (incluye carta aprobación + simulador jun-2026).
...
## 2026-08-31 — LUZ VERDE: correcciones nocturnas aplicadas + limpieza Felipe De La Cuadra
...
## 2026-09-01 — CALCULADORA DE CRÉDITO MÁXIMO (nuevo módulo /calculadora)
- Link directo: {PREVIEW_URL}/calculadora — clave ejecutivos en backend/.env CALC_MAX_CLAVE="CM2026MAX". Ruta pública /api/calcmax en auth.py (clave validada por request).
- Backend (server.py): POST /calcmax/login {clave} → UF día; POST /calcmax/ocr (Form clave+tipo+foto imagen/PDF ≤15MB) → ocr_service + ai_extract.extraer_datos_financieros → campos {renta_titular, renta_codeudor, deuda_cmf_total/codeudor, credito_interno_pav, edad_cliente, credito_solicitado_uf, rut, con_subsidio}; POST /calcmax/calcular → ce.simular_credito (algoritmo espejo, umbrales constitución DashAI) + log db.calcmax_log.
- REGLA CARGA CMF (definida por el dueño en esta sesión): total deuda CMF+PAV prorrateado a 36 meses al 2% ANUAL (amortización francesa) = cuota mensual = carga PRESENTE; carga TOTAL = presente + futura (dividendo nuevo crédito). Ingreso = líquido a pagar. (OJO: distinto del estándar 2% mensual de endeudamiento_mensual usado en el resto de la plataforma — el cambio es SOLO en /calcmax.)
...
## 2026-09-01 — Recalculo comparación con crédito EXACTO + crédito efectivo automático
- /calcmax/calcular: si no se ingresa crédito solicitado pero sí valor propiedad → credito_efectivo = propiedad − subsidio − pie/ahorro (regla mesa), mostrado en UI (credito_efectivo_uf).
- Recalculado con montos exactos (Kevin Macaya 1.522 UF del ADN, Rolando 1.995,19) y leyendo 05_codeudor: coinciden 4/9 (Cristian: OCR falló esta corrida). HALLAZGO CLAVE: en los DS19 con subsidio APROBADOS por mesa, el div/renta implícito según los docs de la carpeta era 38–62% (sobre el umbral 35% del espejo) → mesa considera rentas complementarias que NO están en la carpeta, o aplica criterios más flexibles en DS19. La calculadora espejo es MÁS CONSERVADORA que la mesa real en DS19 con subsidio. Pendiente decisión del usuario: ¿umbral distinto para DS19?

## 2026-09-01 — Módulo de claves y registro de ejecutivos (calculadora)
- NUEVO panel admin /calculadora-admin (clave maestra = CALC_MAX_CLAVE del .env): genera claves con algoritmo progresivo (1ª = 0587; luego dificultad creciente: más dígitos, sin secuencias), lista ejecutivos (clave, nombre, RUT, último acceso), activa/desactiva y comparte link+clave por WhatsApp.
- Login calculadora: clave + RUT OBLIGATORIO (validado módulo 11). Primer ingreso (HTTP 428) exige nombre completo y permite elegir clave personal (4-8 dígitos, única). Clave amarrada al RUT (otro RUT → 403). "cambiar clave" disponible dentro de la app. Sesiones 12h (db.calcmax_sesiones); registro de accesos en db.calcmax_accesos; cada cálculo loguea ejecutivo_rut/nombre en calcmax_log.
- Puente permanente con el espejo: cada cálculo lee EN VIVO constitución DashAI + tasas + seguros + UF de la misma DB del sistema; respuesta incluye motor_espejo.sincronizado y umbrales vivos.
- Probado E2E curl: 0587 generado, 428 registro, cambio a clave personal, token→calcular, re-login, rechazo RUT ajeno. Datos de prueba limpiados (0587 quedó disponible). Screenshot admin OK.
