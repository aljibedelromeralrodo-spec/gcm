...
async def _autocorreo_cliente_rechazado(seg, forzar=False, solo_preview=False):
    ...
    try:
        ...
        # ⛔ NORMATIVA CONSTITUCIONAL — RECHAZO TEXTO EXACTO (INAMOVIBLE): el motivo es el
        # texto EXACTO enviado por el canal oficial; sin agregar, modificar ni inventar.
        # Sin texto exacto disponible → se RETIENE con alerta (jamás se inventa un motivo).
        motivo = ((seg.get("motivo_rechazo") or seg.get("motivo") or seg.get("detalle_rechazo") or "").strip()
                  or ((fd or {}).get("resultado_mesa_texto") or "").strip())[:4000]
        if not motivo:
            msg_sin = f"⛔ Rechazo de {nombre} RETENIDO (norma texto exacto): sin texto original del canal oficial"
            if not await db.alertas.find_one({"mensaje": msg_sin, "leida": False}):
                await db.alertas.insert_one({"id": str(uuid.uuid4()), "tipo": "rechazo",
                    "mensaje": msg_sin, "cliente": nombre, "fecha": now_iso(), "leida": False})
            return {"ok": False, "motivo": "sin_texto_exacto_retenido"}
