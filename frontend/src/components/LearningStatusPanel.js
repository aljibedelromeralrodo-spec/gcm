    ...
    <div data-testid="learning-status-panel" style={{ marginBottom: "1.5rem" }}>
      ...
      {emailStats && (
        <div style={{ display: "flex", gap: "0.4rem", marginBottom: "0.5rem", flexWrap: "wrap" }}>
          <div data-testid="imap-status" style={{
            padding: "0.35rem 0.6rem", borderRadius: "6px", fontSize: "0.7rem", fontWeight: 600,
          ...
          {emailStats.aprobaciones > 0 && (
            <div style={{ padding: "0.35rem 0.6rem", borderRadius: "6px", fontSize: "0.7rem", background: "rgba(0,184,148,0.08)", border: "1px solid rgba(0,184,148,0.2)", color: "#00b894", fontWeight: 600 }}>
              <i className="fa fa-check" style={{ marginRight: "0.2rem" }}></i>{emailStats.aprobaciones} Aprobaciones
            </div>
          )}
          {emailStats.rechazos > 0 && (
            <div style={{ padding: "0.35rem 0.6rem", borderRadius: "6px", fontSize: "0.7rem", background: "rgba(214,48,49,0.08)", border: "1px solid rgba(214,48,49,0.2)", color: "#d63031", fontWeight: 600 }}>
              <i className="fa fa-times" style={{ marginRight: "0.2rem" }}></i>{emailStats.rechazos} Rechazos
            </div>
          )}
          {emailStats.observaciones > 0 && (
            <div style={{ padding: "0.35rem 0.6rem", borderRadius: "6px", fontSize: "0.7rem", background: "rgba(253,203,110,0.1)", border: "1px solid rgba(253,203,110,0.3)", color: "#e17055", fontWeight: 600 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "0.4rem", marginBottom: "0.5rem" }}>
        {[
          ...
          <div key={s.key} style={{ padding: "0.5rem", borderRadius: "8px", background: "var(--bg-tertiary)", textAlign: "center" }}>
          ...
          {patternKeys.map(key => {
            ...
            return (
              <div key={key} data-testid={`pattern-${key}`}
                style={{ padding: "0.5rem 0.7rem", borderRadius: "8px", background: "rgba(99,110,114,0.06)", border: "1px solid var(--border)", fontSize: "0.78rem" }}>
      ...
      {patternKeys.length === 0 && !loading && (
        <div style={{ padding: "0.6rem", borderRadius: "8px", background: "rgba(108,92,231,0.06)", border: "1px solid rgba(108,92,231,0.15)", fontSize: "0.78rem", color: "var(--text-secondary)" }}>
