...
export const EmailAutocomplete = ({ value, onChange, style, placeholder, dataTestId }) => {
  ...
  return (
    <div style={{ position: "relative" }}>
      ...
      {open && sugs.length > 0 && (
        <div style={{ position: "absolute", top: "110%", left: 0, right: 0, background: "rgba(15,23,42,0.9)", backdropFilter: "blur(16px)", WebkitBackdropFilter: "blur(16px)", border: "1px solid rgba(212,175,55,0.4)", borderRadius: 4, zIndex: 40, overflow: "hidden", maxHeight: 220, overflowY: "auto" }}>
          {sugs.map((s, i) => (
