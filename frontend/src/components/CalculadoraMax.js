      ...
      {res && (
        <div id="resultado-calcmax" style={{ ...S.card, border: `1.5px solid ${ORO}` }}>
          ...
          {[["UF del día (SII)", `$${fmt(res.valor_uf, 2)}`],
            ...
            ["Seguro desgravamen" + (res.seguros?.codeudor_duplica_desgravamen ? " (x2 codeudor)" : ""), `$${fmt(res.seguros?.seguro_desgravamen)}`],
            ["Seguro incendio y sismo", `$${fmt(res.seguros?.seguro_incendio)}`],
            ["POSIBLE DIVIDENDO TOTAL (con seguros)", `UF ${fmt(res.dividendo_final_uf, 2)} ≈ $${fmt(res.dividendo_final_clp)}`],
