...
export default function ProactiveAlertsPanel() {
  ...
  return (
    <div data-testid="proactive-alerts-panel" style={{ marginBottom: "1.5rem" }}>
      ...
      {alertas.length === 0 ? (
        <div style={{ padding: "0.75rem", borderRadius: "2px", background: "rgba(0,184,148,0.06)", border: "1px solid rgba(0,184,148,0.15)", fontSize: "0.78rem", color: "var(--text-secondary)" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem" }}>
          {alertas.slice(0, 8).map((a, i) => {
            ...
            return (
              <div key={i} data-testid={`alert-${i}`} style={{ padding: "0.6rem 0.8rem", borderRadius: "2px", background: sev.bg, border: `1px solid ${sev.border}`, fontSize: "0.78rem" }}>
