...
const inpS = { background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.15)", borderRadius: 2, padding: "0.55rem 0.8rem", color: "#fff", fontSize: "0.9rem", width: "100%" };
  ...
  return (
    <>
      <button data-testid={`importar-correo-btn-${destino}`} onClick={abrir}
        style={{ background: "rgba(20,184,166,0.15)", border: "1px solid #14b8a6", color: "#2dd4bf", borderRadius: 2, padding: "0.45rem 0.9rem", fontWeight: 700, cursor: "pointer", fontSize: "0.82rem", ...style }}>
      ...
      {open && (
        <div onClick={() => setOpen(false)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.72)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", padding: "3vh 3vw" }}>
          <div data-testid="importar-correo-modal" onClick={e => e.stopPropagation()}
            style={{ background: "rgba(15,23,42,0.92)", backdropFilter: "blur(16px)", WebkitBackdropFilter: "blur(16px)", border: "1px solid rgba(148,163,184,0.25)", borderRadius: 2, padding: "1.4rem", width: "min(820px, 96vw)", maxHeight: "90vh", overflow: "auto", display: "grid", gap: 10, color: "#e2e8f0" }}>
              ...
              <div data-testid="importar-correo-lista" style={{ display: "grid", gap: 6 }}>
                {correos.map((c, i) => (
                  <label key={i} data-testid={`importar-correo-item-${i}`}
                    style={{ display: "flex", alignItems: "flex-start", gap: 12, padding: "0.8rem 1rem", background: sel[i] ? "rgba(20,184,166,0.12)" : "rgba(255,255,255,0.04)", border: `1px solid ${sel[i] ? "#14b8a6" : "rgba(255,255,255,0.1)"}`, borderRadius: 2, cursor: c.message_id ? "pointer" : "default", fontSize: "1rem" }}>
            ...
            <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
              <button data-testid="importar-correo-cerrar" onClick={() => setOpen(false)}
                style={{ background: "rgba(255,255,255,0.1)", border: "none", color: "#fff", borderRadius: 2, padding: "0.5rem 1rem", fontWeight: 700, cursor: "pointer" }}>Cerrar</button>
              <button data-testid="importar-correo-ejecutar" onClick={importar} disabled={importando}
                style={{ background: "#14b8a6", border: "none", color: "#04211d", borderRadius: 2, padding: "0.5rem 1.1rem", fontWeight: 800, cursor: "pointer" }}>
