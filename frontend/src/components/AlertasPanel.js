...
export default function AlertasPanel() {
  ...
  return (
    <div data-testid="alertas-panel" style={{ marginBottom: "1.5rem", padding: "1rem", borderRadius: "0px", background: alertas.length > 0 ? "rgba(245,158,11,0.08)" : "var(--bg-card)", border: `1px solid ${alertas.length > 0 ? "rgba(245,158,11,0.3)" : "var(--border)"}` }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "0.75rem" }}>
        ...
        <select data-testid="alertas-dias-select" value={dias} onChange={e => setDias(parseInt(e.target.value))}
          style={{ padding: "0.3rem 0.6rem", borderRadius: "0px", border: "1px solid var(--border)", background: "var(--bg-hover)", color: "var(--text-primary)", fontSize: "0.8rem" }}>
