...
export default function AlertasPanel() {
  ...
  return (
    <div data-testid="alertas-panel" style={{ marginBottom: "1.5rem", padding: "1rem", borderRadius: "4px", background: alertas.length > 0 ? "rgba(245,158,11,0.08)" : "var(--bg-card)", border: `1px solid ${alertas.length > 0 ? "rgba(245,158,11,0.3)" : "var(--border)"}` }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "0.75rem" }}>
        ...
        <select data-testid="alertas-dias-select" value={dias} onChange={e => setDias(parseInt(e.target.value))}
          style={{ padding: "0.3rem 0.6rem", borderRadius: "4px", border: "1px solid var(--border)", background: "var(--bg-hover)", color: "var(--text-primary)", fontSize: "0.8rem" }}>
          ...
          {alertas.map((a, i) => {
            ...
            return (
              <div key={i} data-testid={`alerta-row-${i}`} style={{ display: "flex", alignItems: "center", gap: "0.75rem", padding: "0.5rem 0", borderBottom: i < alertas.length - 1 ? "1px solid var(--border)" : "none" }}>
                <div style={{ width: "32px", height: "32px", borderRadius: "4px", background: diasSin > 14 ? "rgba(239,68,68,0.15)" : "rgba(245,158,11,0.15)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                ...
                <span style={{ padding: "2px 8px", borderRadius: "4px", fontSize: "0.7rem", fontWeight: 600, background: "rgba(99,102,241,0.12)", color: "#6366f1", whiteSpace: "nowrap" }}>{a.estado || "en proceso"}</span>
              </div>
            );
          })}
        </div>
