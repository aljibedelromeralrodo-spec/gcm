...
export default function WhatsAppModule() {
  ...
  return (
    <div className="module-content" data-testid="whatsapp-module">
      ...
      <div data-testid="portal-ejecutivos-card" style={{ background: "var(--bg-card)", border: "1px solid rgba(212,175,55,0.4)", borderRadius: 0, padding: "1.25rem", marginBottom: "1.5rem" }}>
        ...
        <button data-testid="btn-copy-portal-link" onClick={() => { navigator.clipboard.writeText(window.location.origin); alert("Link copiado. Envíalo a tus ejecutivos por WhatsApp."); }}
          style={{ background: "var(--gold, #d4af37)", color: "#0a0e17", border: "none", borderRadius: 0, padding: "0.5rem 1rem", fontWeight: 700, cursor: "pointer", marginRight: "0.6rem" }}>
        ...
        <a data-testid="btn-share-portal-wa" href={`https://wa.me/?text=${encodeURIComponent("Instala la app de Central Mutuos para enviar los documentos de tus clientes directo al sistema: " + window.location.origin + " — Abrila en Chrome y tocá 'Agregar a pantalla de inicio'.")}`} target="_blank" rel="noreferrer"
          style={{ display: "inline-block", background: "#25d366", color: "#fff", borderRadius: 0, padding: "0.5rem 1rem", fontWeight: 700, textDecoration: "none" }}>
      ...
      <div style={{ background: "var(--bg-card)", border: `1px solid ${isConnected ? "rgba(16,185,129,0.5)" : "rgba(245,158,11,0.4)"}`, borderRadius: 0, padding: "1.25rem", marginBottom: "1.5rem" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "0.75rem" }}>
          ...
          <button onClick={refreshAll} title="Refrescar" style={{ padding: "0.5rem 0.75rem", background: "rgba(255,255,255,0.04)", border: "1px solid var(--border)", borderRadius: 0, color: "var(--text-secondary)", cursor: "pointer" }}>
        ...
        {/* QR Code */}
        {hasQR && qrData?.qrCode && (
          <div data-testid="wa-qr-container" style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "1.5rem", background: "rgba(255,255,255,0.03)", borderRadius: 0, gap: "0.85rem" }}>
            <div style={{ background: "#fff", padding: "0.75rem", borderRadius: 0 }}>
        ...
        {isConnected && (
          <div style={{ marginTop: "1rem", display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
            <input type="text" value={testMessage} onChange={e => setTestMessage(e.target.value)}
              ...
              style={{ flex: 1, minWidth: 200, padding: "0.55rem 0.85rem", background: "var(--bg-input)", border: "1px solid var(--border)", borderRadius: 0, color: "var(--text-primary)", fontSize: "0.85rem" }} />
            <button onClick={sendTest} disabled={loading} data-testid="wa-test-btn"
              style={{ padding: "0.55rem 1rem", background: "linear-gradient(135deg, #25d366, #128c7e)", color: "#fff", border: "none", borderRadius: 0, fontWeight: 700, cursor: loading ? "wait" : "pointer", fontSize: "0.85rem" }}>
      ...
      {/* Approval Queue */}
      <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 0, padding: "1.25rem" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1rem" }}>
          <h3 style={{ color: "var(--gold)", fontSize: "1rem", margin: 0, display: "flex", alignItems: "center", gap: 8 }}>
            <i className="fa fa-tasks"></i>
            Bandeja de Aprobaciones
            <span style={{ background: approvals.length > 0 ? "#f59e0b" : "rgba(212,175,55,0.2)", color: approvals.length > 0 ? "#0a0e17" : "var(--text-muted)", borderRadius: 0, padding: "1px 9px", fontSize: "0.7rem", fontWeight: 700 }}>
              {approvals.length}
            </span>
          </h3>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }} data-testid="wa-approvals-list">
            {approvals.map(a => (
              <div key={a.id} data-testid={`approval-${a.id}`}
                style={{ background: "rgba(245,158,11,0.06)", border: "1px solid rgba(245,158,11,0.3)", borderRadius: 0, padding: "0.85rem 1rem" }}>
                ...
                {a.suggested_text && (
                  <div style={{ background: "rgba(0,0,0,0.25)", borderRadius: 0, padding: "0.55rem 0.75rem", fontSize: "0.8rem", color: "var(--text-secondary)", fontStyle: "italic", marginBottom: 8 }}>
                    "{a.suggested_text}"
                  </div>
                )}
