  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: "'Segoe UI', sans-serif", display: "flex", flexDirection: "column", alignItems: "center" }}>
      ...
      <div style={{ maxWidth: "520px", width: "90%", marginTop: "3rem" }}>
        <div style={{ padding: "2rem", borderRadius: "4px", background: C.card, border: `1px solid ${C.border}` }}>
          ...
          <div style={{ display: "flex", gap: "0.5rem" }}>
            <input data-testid="portal-rut-input" value={rut} onChange={e => setRut(e.target.value)} onKeyDown={e => e.key === "Enter" && consultar()}
              ...
              style={{ flex: 1, padding: "0.8rem 1rem", borderRadius: "4px", border: `1px solid ${C.border}`, background: "rgba(255,255,255,0.05)", color: C.text, fontSize: "1rem", outline: "none" }} />
            <button data-testid="portal-search-btn" onClick={consultar} disabled={loading}
              style={{ padding: "0.8rem 1.5rem", borderRadius: "4px", background: `linear-gradient(135deg, ${C.accent}, #b8860b)`, color: "#000", fontWeight: 700, fontSize: "0.9rem", border: "none", cursor: "pointer" }}>
        ...
        {/* No results */}
        {data && !data.found && (
          <div data-testid="portal-not-found" style={{ marginTop: "1.5rem", padding: "2rem", borderRadius: "4px", background: C.card, border: `1px solid ${C.border}`, textAlign: "center" }}>
        ...
        {data && data.found && (
          <div data-testid="portal-results" style={{ marginTop: "1.5rem" }}>
            ...
            {data.operaciones?.length > 0 && data.operaciones.map((op, i) => {
              ...
              return (
                <div key={i} data-testid={`portal-op-${i}`} style={{ marginBottom: "1rem", padding: "1.25rem", borderRadius: "4px", background: C.card, border: `1px solid ${C.border}` }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "0.75rem" }}>
                    <div style={{ fontWeight: 700, color: C.text, fontSize: "1rem" }}>{op.cliente_display || op.id}</div>
                    <span style={{ padding: "0.3rem 0.8rem", borderRadius: "4px", background: `${est.color}20`, color: est.color, fontWeight: 700, fontSize: "0.8rem" }}>
                  ...
                  {op.resumen && (
                    <div style={{ marginTop: "0.75rem", padding: "0.6rem", borderRadius: "4px", background: "rgba(255,255,255,0.03)", fontSize: "0.8rem", color: C.muted, lineHeight: 1.5 }}>
            ...
            {data.simulaciones?.length > 0 && (
              <div style={{ marginTop: "0.5rem", padding: "1rem", borderRadius: "4px", background: C.card, border: `1px solid ${C.border}` }}>
                ...
                {data.simulaciones.map((s, i) => (
                  <div key={i} style={{ display: "flex", alignItems: "center", gap: "0.75rem", padding: "0.5rem 0", borderBottom: i < data.simulaciones.length - 1 ? `1px solid ${C.border}` : "none" }}>
                    <span style={{ padding: "2px 8px", borderRadius: "4px", fontSize: "0.75rem", fontWeight: 700, background: s.precalificacion_aprobada ? `${C.green}20` : `${C.red}20`, color: s.precalificacion_aprobada ? C.green : C.red }}>
