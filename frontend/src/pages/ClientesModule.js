...
const BrokersPanel = ({ brokers, dest, setDest, reloadBrokers, soloAdmin }) => {
  ...
  const inpS = { padding: "0.4rem 0.6rem", borderRadius: 4, border: "1px solid rgba(148,163,184,0.3)", background: "#1e293b", color: "#e2e8f0", fontSize: 12 };
  return (
    <div data-testid="brokers-panel" style={{ background: "rgba(212,175,55,0.06)", border: "1px solid rgba(212,175,55,0.25)", borderRadius: 4, padding: "0.7rem 0.9rem" }}>
      ...
      {showAdd ? (
        ...
        <button onClick={() => setShowAdd(true)} data-testid="broker-add-btn" style={{ marginTop: 8, background: "transparent", border: "1px dashed rgba(212,175,55,0.5)", color: "#93c5fd", borderRadius: 4, padding: "0.3rem 0.7rem", fontSize: 11.5, cursor: "pointer" }}>
  ...
  return (
    <div>
      <input
        ...
        style={{ width: "100%", padding: "0.4rem 0.5rem", borderRadius: 4, border: "1px solid #d4d4d8", fontSize: 13, color: "#000", fontWeight: 600, background: "#fff" }}
      />
      {preview && (
        <div style={{ fontSize: 10, color: "#0369a1", marginTop: 2, fontWeight: 500 }}>{preview}</div>
      )}
    </div>
  );
}
    ...
    <div className="clientes-module" data-testid="clientes-module">
        ...
        <div data-testid="clientes-list">
          ...
          <div data-testid="folder-tabs" style={{ display: "flex", gap: 8, margin: "0.9rem 0" }}>
            <button data-testid="tab-clientes" onClick={() => setFolderTab("clientes")}
              style={{ padding: "0.55rem 1.2rem", borderRadius: 4, fontWeight: 800, fontSize: 13, cursor: "pointer",
          ...
          <div className="clientes-grid">
            ...
            {filtered.map(f => {
              ...
              const modBtn = (bg, border, color, big) => ({
                ...
                borderRadius: 4, border: `1.5px solid ${border}`, background: bg, color,
              ...
              return (
                <div key={f.id} className="clientes-card" data-testid={`folder-${f.id}`} style={cardStyle}>
                  {enviadoManual && (
                    <div style={{ position: "absolute", top: 8, left: 8, background: "#fff", color: "#dc2626", borderRadius: 4, padding: "2px 8px", fontSize: 10, fontWeight: 800 }} data-testid={`badge-enviado-${f.id}`}>
                      ✅ ENVIADO (manual)
                    </div>
                  )}
                  {missing.length > 0 && (
                  ...
                  <div className="clientes-card-info">
                    ...
                    <div style={{ display: "flex", gap: 6, marginTop: 4, flexWrap: "wrap" }}>
                      ...
                      {f.emails_sent_count > 0 && (
                        <span title={`Último envío: ${(f.last_email_sent_at || "").slice(0,19).replace('T',' ')}`} style={{ fontSize: 10, background: "rgba(212,175,55,0.2)", color: "#1d4ed8", padding: "2px 6px", borderRadius: 4, fontWeight: 700 }}>📧 Enviado a mesa × {f.emails_sent_count}{f.last_email_sent_at ? ` · ${fmtAct(f.last_email_sent_at)}` : ""}</span>
                      )}
                      {hasFin ? (
                    ...
                    {missing.length > 0 && (
                      <div data-testid={`missing-docs-${f.id}`} style={{ display: "flex", gap: 4, marginTop: 5, flexWrap: "wrap", alignItems: "center" }}>
                        ...
                        {missing.map(m => (
                          <span key={m} style={{ fontSize: 10, fontWeight: 700, background: enviadoManual ? "rgba(255,255,255,0.25)" : "rgba(220,38,38,0.15)", color: enviadoManual ? "#fff" : "#dc2626", padding: "2px 7px", borderRadius: 4, border: enviadoManual ? "1px solid rgba(255,255,255,0.4)" : "1px solid rgba(220,38,38,0.35)" }}>
                            {CAT_LABELS[m] || m}
                          </span>
                        ))}
                      </div>
                    ...
                    {(f.criterios || []).length > 0 && (
                      <div data-testid={`criterios-list-${f.id}`} style={{ display: "flex", gap: 4, marginTop: 5, flexWrap: "wrap", alignItems: "center" }}>
                        {f.criterios.map(c => (
                          <span key={c.nombre} title={c.nombre} style={{ fontSize: 10, fontWeight: 700, padding: "2px 7px", borderRadius: 4,
                    ...
                    <div style={{ display: "flex", gap: 8, marginTop: 5, alignItems: "center", flexWrap: "wrap" }}>
                      ...
                      {missing.length > 0 && f.source_email && (
                        <button data-testid={`btn-pedir-faltantes-${f.id}`} onClick={() => openPedirFaltantes(f)}
                          style={{ fontSize: 10.5, fontWeight: 800, padding: "3px 10px", borderRadius: 4, cursor: "pointer",
                  ...
                  {f.prob_aprobacion && f.prob_aprobacion.porcentaje != null && (
                    <div style={{ display: "flex", gap: 8, alignItems: "stretch", flexShrink: 0 }}>
                      <div data-testid={`prob-aprobacion-${f.id}`}
                        ...
                        style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minWidth: 88, padding: "6px 10px", borderRadius: 4,
                      ...
                      <div data-testid={`mesa-criterios-${f.id}`}
                        style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minWidth: 92, padding: "6px 10px", borderRadius: 4,
      ...
      {view === "detail" && currentFolder && (
        <div data-testid="clientes-detail">
          <div className="clientes-detail-header">
            ...
            <div className="clientes-detail-actions">
              ...
              <button className="docs-btn secondary" onClick={() => openFinPanel(currentFolder)} data-testid="btn-fin-detail"
                style={{ background: "rgba(14,165,233,0.15)", border: "1px solid #0ea5e9", color: "#0369a1" }}>
              ...
              <button className="docs-btn secondary" onClick={() => window.open(`${API}/api/informes/vip/${currentFolder.id}/pdf`, "_blank")} data-testid={`btn-informe-vip-${currentFolder.id}`}
                style={{ background: "rgba(212,175,55,0.12)", border: "1px solid rgba(212,175,55,0.5)", color: "#93c5fd" }}>
          ...
          {finOpenId === currentFolder.id && (
            <div data-testid={`fin-detail-panel`} style={{ background: "rgba(14,165,233,0.08)", border: "1px solid #0ea5e9", borderRadius: 4, padding: "0.9rem", marginBottom: "0.8rem" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.6rem" }}>
                <h5 style={{ margin: 0, color: "#0369a1" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: "0.5rem" }}>
                {currentFolder.source_email && (() => {
                  ...
                  return (
                    <div data-testid="fin-origen" style={{ gridColumn: "1 / -1", background: "#dbeafe", border: "1px solid #93c5fd", borderRadius: 4, padding: "0.5rem 0.7rem", fontSize: 12 }}>
                      <b style={{ color: "#1e3a8a" }}>📧 Origen de la solicitud:</b> <span style={{ color: "#0369a1", fontFamily: "monospace" }}>{disp}</span>
                    </div>
                  );
                })()}
                <label style={{ fontSize: 12, color: "#dc2626", fontWeight: 700 }}><b style={{ display: "block", color: "#dc2626", fontWeight: 800 }}>Inmobiliaria</b>
                  <input type="text" value={finDraft.inmobiliaria || ""} onChange={(e) => setFinDraft({ ...finDraft, inmobiliaria: e.target.value })} style={{ width: "100%", padding: "0.4rem 0.5rem", borderRadius: 4, border: "1px solid #94a3b8", fontSize: 13, color: "#000", fontWeight: 600, background: "#fff" }} />
                </label>
                <label style={{ fontSize: 12, color: "#dc2626", fontWeight: 700 }}><b style={{ display: "block", color: "#dc2626", fontWeight: 800 }}>Proyecto</b>
                  <input type="text" value={finDraft.proyecto || ""} onChange={(e) => setFinDraft({ ...finDraft, proyecto: e.target.value })} style={{ width: "100%", padding: "0.4rem 0.5rem", borderRadius: 4, border: "1px solid #94a3b8", fontSize: 13, color: "#000", fontWeight: 600, background: "#fff" }} />
                </label>
                <label style={{ fontSize: 12, gridColumn: "1 / -1", color: "#dc2626", fontWeight: 700 }}><b style={{ display: "block", color: "#dc2626", fontWeight: 800 }}>Tipo de operación</b>
                  <select value={finDraft.con_subsidio ? "con" : "sin"} onChange={(e) => setFinDraft({ ...finDraft, con_subsidio: e.target.value === "con" })} style={{ width: "100%", padding: "0.4rem 0.5rem", borderRadius: 4, border: "1px solid #94a3b8", fontSize: 13, color: "#000", fontWeight: 600, background: "#fff" }}>
                ...
                <label style={{ fontSize: 12, color: "#dc2626", fontWeight: 700 }}><b style={{ display: "block", color: "#dc2626", fontWeight: 800 }}>Tipo propiedad</b>
                  <select value={finDraft.tipo_propiedad || ""} onChange={(e) => setFinDraft({ ...finDraft, tipo_propiedad: e.target.value })} style={{ width: "100%", padding: "0.4rem 0.5rem", borderRadius: 4, border: "1px solid #94a3b8", fontSize: 13, color: "#000", fontWeight: 600, background: "#fff" }}>
                ...
                <label style={{ fontSize: 12, color: "#dc2626", fontWeight: 700 }}><b style={{ display: "block", color: "#dc2626", fontWeight: 800 }}>Fecha de entrega</b>
                  <select value={finDraft.fecha_entrega || ""} onChange={(e) => setFinDraft({ ...finDraft, fecha_entrega: e.target.value })} data-testid="fin-fecha-entrega-detail" style={{ width: "100%", padding: "0.4rem 0.5rem", borderRadius: 4, border: "1px solid #94a3b8", fontSize: 13, color: "#000", fontWeight: 600, background: "#fff" }}>
                    <option value="">— seleccionar —</option>
                ...
                <label style={{ fontSize: 12, color: "#dc2626", fontWeight: 700 }}><b style={{ display: "block", color: "#dc2626", fontWeight: 800 }}>Valor propiedad (UF)</b>
                  <input type="number" step="0.01" value={finDraft.valor_propiedad || ""} onChange={(e) => setFinDraft({ ...finDraft, valor_propiedad: e.target.value })} style={{ width: "100%", padding: "0.35rem", borderRadius: 4, border: "1px solid #d4d4d8", fontSize: 13, color: "#000", fontWeight: 600, background: "#fff" }} />
                </label>
                {finDraft.con_subsidio && (
                  <label style={{ fontSize: 12, color: "#dc2626", fontWeight: 700 }}><b style={{ display: "block", color: "#dc2626", fontWeight: 800 }}>Subsidio (UF)</b>
                    <input type="number" step="0.01" value={finDraft.monto_subsidio || ""} onChange={(e) => setFinDraft({ ...finDraft, monto_subsidio: e.target.value })} style={{ width: "100%", padding: "0.35rem", borderRadius: 4, border: "1px solid #d4d4d8", fontSize: 13, color: "#000", fontWeight: 600, background: "#fff" }} />
                  </label>
                )}
                {finDraft.con_subsidio && (
                  <label style={{ fontSize: 12, color: "#dc2626", fontWeight: 700 }}><b style={{ display: "block", color: "#dc2626", fontWeight: 800 }}>Ahorro (UF)</b>
                    <input type="number" step="0.01" value={finDraft.ahorro || ""} onChange={(e) => setFinDraft({ ...finDraft, ahorro: e.target.value })} style={{ width: "100%", padding: "0.35rem", borderRadius: 4, border: "1px solid #d4d4d8", fontSize: 13, color: "#000", fontWeight: 600, background: "#fff" }} />
                  </label>
                )}
                {!finDraft.con_subsidio && (
                  <label style={{ fontSize: 12, color: "#dc2626", fontWeight: 700 }}><b style={{ display: "block", color: "#dc2626", fontWeight: 800 }}>Pie (UF)</b>
                    <input type="number" step="0.01" value={finDraft.monto_pie || ""} onChange={(e) => setFinDraft({ ...finDraft, monto_pie: e.target.value })} style={{ width: "100%", padding: "0.35rem", borderRadius: 4, border: "1px solid #d4d4d8", fontSize: 13, color: "#000", fontWeight: 600, background: "#fff" }} />
                  </label>
                )}
                <label style={{ fontSize: 12, color: "#dc2626", fontWeight: 700 }}><b style={{ display: "block", color: "#dc2626", fontWeight: 800 }}>Monto crédito (UF)</b>
                  <input type="number" step="0.01" value={finDraft.monto_credito || ""} onChange={(e) => setFinDraft({ ...finDraft, monto_credito: e.target.value })} style={{ width: "100%", padding: "0.35rem", borderRadius: 4, border: "1px solid #d4d4d8", fontSize: 13, color: "#000", fontWeight: 600, background: "#fff" }} />
                </label>
                <label style={{ fontSize: 12, color: "#dc2626", fontWeight: 700 }}><b style={{ display: "block", color: "#dc2626", fontWeight: 800 }}>Reserva (UF)</b>
                  <input type="number" step="0.01" value={finDraft.monto_reserva || ""} onChange={(e) => setFinDraft({ ...finDraft, monto_reserva: e.target.value })} style={{ width: "100%", padding: "0.35rem", borderRadius: 4, border: "1px solid #d4d4d8", fontSize: 13, color: "#000", fontWeight: 600, background: "#fff" }} />
                </label>
              </div>
              ...
              {(() => {
                ...
                return (
                  <div style={{ marginTop: 8, display: "grid", gap: 6 }}>
                    <div style={{ padding: "0.7rem 0.9rem", borderRadius: 4, background: ok ? "#dcfce7" : "#fee2e2", color: ok ? "#166534" : "#991b1b", fontSize: 14.5, fontWeight: 700, lineHeight: 1.45 }}>
                      {ok
                    ...
                    {!finDraft.con_subsidio && mc > 0 && (
                      <div style={{ padding: "0.6rem 0.8rem", borderRadius: 4, background: excedeMax ? "#fee2e2" : "#e0f2fe", color: excedeMax ? "#991b1b" : "#075985", fontSize: 13.5, fontWeight: 600, lineHeight: 1.45 }}>
                        {excedeMax
            ...
            {(() => {
              ...
              return (
                <>
                  ...
                  {codFiles.length > 0 && (
                    <div data-testid="codeudor-subfolder" style={{ marginTop: 14, border: "1.5px dashed #8b5cf6", borderRadius: 4, padding: "10px 12px", background: "rgba(139,92,246,0.06)" }}>
                  ...
                  {estudioFiles.length > 0 && (
                    <div data-testid="estudio-subfolder" style={{ marginTop: 14, border: "1.5px dashed #14b8a6", borderRadius: 4, padding: "10px 12px", background: "rgba(20,184,166,0.06)" }}>
      ...
      {view === "ajustes" && (
        <div data-testid="clientes-ajustes">
          ...
          <div style={{ padding: "1rem", background: "rgba(212,175,55,0.08)", border: "1px solid #d4af37", borderRadius: 4, marginBottom: "1rem", fontSize: "0.88rem", lineHeight: 1.6 }}>
                  ...
                  <div key={f.id} className="clientes-card" style={{ padding: "1rem", flexDirection: "column", alignItems: "stretch", borderLeft: cr.manual_override ? "3px solid #8b5cf6" : "" }} data-testid={`ajuste-${f.id}`}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.5rem", gap: "0.5rem", flexWrap: "wrap" }}>
                      <h4 style={{ margin: 0 }}>
                        ...
                        {cr.manual_override && (
                          <span data-testid={`manual-badge-${f.id}`} style={{ background: "#ede9fe", color: "#6d28d9", padding: "2px 8px", borderRadius: 4, fontSize: 11, fontWeight: 700, marginLeft: 8 }}>
                      ...
                      {!isEditing && (
                        <div style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center" }}>
                          <span style={{ background: cr.is_request ? "#dcfce7" : "#fee2e2", color: cr.is_request ? "#166534" : "#991b1b", padding: "2px 8px", borderRadius: 4, fontSize: 12, fontWeight: 700 }}>
                            {cr.is_request ? "✓ Solicitud" : "✗ No"}
                          </span>
                          <span style={{ background: "#fef3c7", color: "#92400e", padding: "2px 8px", borderRadius: 4, fontSize: 12 }}>{cr.client_type || "?"}</span>
                          <span style={{ background: "#e0e7ff", color: "#3730a3", padding: "2px 8px", borderRadius: 4, fontSize: 12 }}>{sub.tipo || "?"}</span>
                          {cod.has_codeudor && <span style={{ background: "#fce7f3", color: "#9d174d", padding: "2px 8px", borderRadius: 4, fontSize: 12 }}>+ codeudor {cod.name || "s/n"}</span>}
                          ...
                          <button className="docs-btn secondary" style={{ padding: "0.3rem 0.6rem", fontSize: 12, background: "#0ea5e9", color: "#fff", border: "none" }} onClick={() => openFinPanel(f)} data-testid={`btn-fin-${f.id}`}>
                    ...
                    {isEditing && (
                      <div data-testid={`edit-form-${f.id}`} style={{ background: "rgba(139,92,246,0.08)", border: "1px solid #8b5cf6", borderRadius: 4, padding: "0.9rem", marginBottom: "0.6rem", display: "grid", gap: "0.7rem", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))" }}>
                        <label style={{ fontSize: 12 }}>
                          <b style={{ display: "block", marginBottom: 4 }}>¿Es solicitud?</b>
                          <select value={editDraft.is_request ? "si" : "no"} onChange={(e) => setEditDraft({ ...editDraft, is_request: e.target.value === "si" })} data-testid={`edit-is-request-${f.id}`} style={{ width: "100%", padding: "0.35rem", borderRadius: 4, border: "1px solid #d4d4d8" }}>
                        ...
                        <label style={{ fontSize: 12 }}>
                          ...
                          <select value={editDraft.client_type} onChange={(e) => setEditDraft({ ...editDraft, client_type: e.target.value })} data-testid={`edit-client-type-${f.id}`} style={{ width: "100%", padding: "0.35rem", borderRadius: 4, border: "1px solid #d4d4d8" }}>
                        ...
                        <label style={{ fontSize: 12 }}>
                          ...
                          <select value={editDraft.subsidy_tipo} onChange={(e) => setEditDraft({ ...editDraft, subsidy_tipo: e.target.value })} data-testid={`edit-subsidy-${f.id}`} style={{ width: "100%", padding: "0.35rem", borderRadius: 4, border: "1px solid #d4d4d8" }}>
                        ...
                        <label style={{ fontSize: 12 }}>
                          ...
                          <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                            ...
                            <input type="text" placeholder="Nombre codeudor" value={editDraft.codeudor_name || ""} onChange={(e) => setEditDraft({ ...editDraft, codeudor_name: e.target.value, codeudor_has: true })} data-testid={`edit-codeudor-name-${f.id}`} style={{ flex: 1, padding: "0.35rem", borderRadius: 4, border: "1px solid #d4d4d8", fontSize: 12 }} />
                          </div>
                        </label>
                    ...
                    {/* ARCHIVOS TOOLBAR */}
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 6, padding: "0.4rem 0.6rem", background: "rgba(15,23,42,0.4)", borderRadius: 4, border: "1px solid rgba(148,163,184,0.15)" }}>
                      ...
                      <button
                        ...
                        style={{ background: "rgba(34,197,94,0.15)", border: "1px solid rgba(34,197,94,0.5)", color: "#4ade80", borderRadius: 4, padding: "4px 10px", cursor: "pointer", fontSize: 12 }}
                      ...
                      <button
                        ...
                        style={{ background: "rgba(251,146,60,0.15)", border: "1px solid rgba(251,146,60,0.5)", color: "#fdba74", borderRadius: 4, padding: "4px 10px", cursor: "pointer", fontSize: 12 }}
                      ...
                      <button
                        ...
                        style={{ background: "rgba(250,204,21,0.15)", border: "1px solid rgba(250,204,21,0.5)", color: "#facc15", borderRadius: 4, padding: "4px 10px", cursor: "pointer", fontSize: 12, fontWeight: 600 }}
                      ...
                      <button
                        ...
                        style={{ background: "rgba(168,85,247,0.15)", border: "1px solid rgba(168,85,247,0.5)", color: "#c084fc", borderRadius: 4, padding: "4px 10px", cursor: selectionCount(f.id) < 1 ? "not-allowed" : "pointer", opacity: selectionCount(f.id) < 1 ? 0.5 : 1, fontSize: 12 }}
                      ...
                      <button
                        ...
                        style={{ background: "rgba(212,175,55,0.18)", border: "1px solid rgba(212,175,55,0.5)", color: "#93c5fd", borderRadius: 4, padding: "4px 10px", cursor: "pointer", fontSize: 12 }}
                      ...
                      {selectionCount(f.id) > 0 && (
                        <button
                          ...
                          style={{ background: "transparent", border: "1px solid rgba(148,163,184,0.3)", color: "#94a3b8", borderRadius: 4, padding: "4px 10px", cursor: "pointer", fontSize: 11 }}
                    ...
                    {finOpenId === f.id && (
                      <div data-testid={`fin-panel-${f.id}`} style={{ background: "rgba(14,165,233,0.08)", border: "1px solid #0ea5e9", borderRadius: 4, padding: "0.9rem", marginBottom: "0.6rem" }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.6rem" }}>
                          <h5 style={{ margin: 0, color: "#0369a1" }}><i className="fa fa-dollar" /> Datos financieros del cliente</h5>
                          <div style={{ display: "flex", gap: 6 }}>
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: "0.5rem" }}>
                          <label style={{ fontSize: 12, color: "#dc2626", fontWeight: 700 }}>
                            ...
                            <input type="text" value={f.nombre} readOnly style={{ width: "100%", padding: "0.4rem 0.5rem", borderRadius: 4, background: "#f1f5f9", border: "1px solid #94a3b8", fontSize: 13, color: "#000", fontWeight: 600 }} />
                          </label>
                          <label style={{ fontSize: 12, color: "#dc2626", fontWeight: 700 }}>
                            <b style={{ display: "block", color: "#dc2626", fontWeight: 800 }}>RUT</b>
                            <input type="text" value={f.rut || ""} readOnly style={{ width: "100%", padding: "0.4rem 0.5rem", borderRadius: 4, background: "#f1f5f9", border: "1px solid #94a3b8", fontSize: 13, color: "#000", fontWeight: 600 }} />
                          </label>
                          <label style={{ fontSize: 12, color: "#dc2626", fontWeight: 700 }}>
                            <b style={{ display: "block", color: "#dc2626", fontWeight: 800 }}>Proyecto</b>
                            <input type="text" value={finDraft.proyecto || ""} onChange={(e) => setFinDraft({ ...finDraft, proyecto: e.target.value })} data-testid={`fin-proyecto-${f.id}`} style={{ width: "100%", padding: "0.4rem 0.5rem", borderRadius: 4, border: "1px solid #94a3b8", fontSize: 13, color: "#000", fontWeight: 600, background: "#fff" }} />
                          </label>
                          <label style={{ fontSize: 12, color: "#dc2626", fontWeight: 700 }}>
                            <b style={{ display: "block", color: "#dc2626", fontWeight: 800 }}>Inmobiliaria</b>
                            <input type="text" value={finDraft.inmobiliaria || ""} onChange={(e) => setFinDraft({ ...finDraft, inmobiliaria: e.target.value })} data-testid={`fin-inmobiliaria-${f.id}`} style={{ width: "100%", padding: "0.4rem 0.5rem", borderRadius: 4, border: "1px solid #94a3b8", fontSize: 13, color: "#000", fontWeight: 600, background: "#fff" }} />
                          </label>
                          <label style={{ fontSize: 12, gridColumn: "1 / -1", color: "#dc2626", fontWeight: 700 }}>
                            ...
                            <select value={finDraft.con_subsidio ? "con" : "sin"} onChange={(e) => setFinDraft({ ...finDraft, con_subsidio: e.target.value === "con" })} data-testid={`fin-subsidy-${f.id}`} style={{ width: "100%", padding: "0.4rem 0.5rem", borderRadius: 4, border: "1px solid #94a3b8", fontSize: 13, color: "#000", fontWeight: 600, background: "#fff" }}>
                              <option value="con">CON subsidio (DS1 / DS19)</option>
                              <option value="sin">SIN subsidio</option>
                            </select>
                          </label>
                          <label style={{ fontSize: 12, color: "#dc2626", fontWeight: 700 }}>
                            ...
                            <select value={finDraft.tipo_propiedad || ""} onChange={(e) => setFinDraft({ ...finDraft, tipo_propiedad: e.target.value })} data-testid={`fin-tipo-prop-${f.id}`} style={{ width: "100%", padding: "0.4rem 0.5rem", borderRadius: 4, border: "1px solid #94a3b8", fontSize: 13, color: "#000", fontWeight: 600, background: "#fff" }}>
                          ...
                          <label style={{ fontSize: 12, color: "#dc2626", fontWeight: 700 }}>
                            ...
                            <select value={finDraft.fecha_entrega || ""} onChange={(e) => setFinDraft({ ...finDraft, fecha_entrega: e.target.value })} data-testid={`fin-fecha-entrega-${f.id}`} style={{ width: "100%", padding: "0.4rem 0.5rem", borderRadius: 4, border: "1px solid #94a3b8", fontSize: 13, color: "#000", fontWeight: 600, background: "#fff" }}>
                              <option value="">— seleccioná —</option>
                              <option value="inmediata">Inmediata</option>
                              <option value="futura">Futura</option>
                            </select>
                        ...
                        {(() => {
                          ...
                          return (
                            <div style={{ marginTop: 8, display: "grid", gap: 6 }}>
                              <div data-testid={`fin-sum-${f.id}`} style={{ padding: "0.7rem 0.9rem", borderRadius: 4, background: ok ? "#dcfce7" : "#fee2e2", color: ok ? "#166534" : "#991b1b", fontSize: 14.5, fontWeight: 700, lineHeight: 1.45 }}>
                                {ok
                                  ? `✅ Suma cuadra: ${suma.toFixed(2)} UF = ${vp.toFixed(2)} UF`
                                  : `⚠️ Diferencia: ${diff.toFixed(2)} UF (suma actual: ${suma.toFixed(2)} vs valor propiedad: ${vp.toFixed(2)}). ${finDraft.con_subsidio ? 'Subsidio + Ahorro + Reserva + Crédito' : 'Pie + Reserva + Crédito'} debe = Valor propiedad.`}
                              </div>
                              {!finDraft.con_subsidio && mc > 0 && (
                                <div data-testid={`fin-80pct-${f.id}`} style={{ padding: "0.6rem 0.8rem", borderRadius: 4, background: excedeMax ? "#fee2e2" : "#e0f2fe", color: excedeMax ? "#991b1b" : "#075985", fontSize: 13.5, fontWeight: 600, lineHeight: 1.45 }}>
                                  {excedeMax
                                    ? `🚫 Crédito ${mc.toFixed(2)} UF supera el 80% máximo (${max80.toFixed(2)} UF) — SIN subsidio no permite más del 80% del valor propiedad. Actual: ${pctCredito.toFixed(1)}%.`
                                    : `📊 Crédito representa ${pctCredito.toFixed(1)}% del valor propiedad (máximo permitido SIN subsidio: 80% = ${max80.toFixed(2)} UF).`}
                                </div>
                        ...
                        <div style={{ marginTop: "0.6rem", textAlign: "right" }}>
                          <button className="docs-btn" style={{ background: "#0ea5e9", color: "#fff", border: "none" }} onClick={() => saveFinPanel(f.id)} data-testid={`btn-fin-save-${f.id}`} disabled={finSaving || ocrRunning}>
                    ...
                    {(f.subfolders || []).map(s => (
                      <details key={s.name} style={{ background: "rgba(255,255,255,0.03)", borderRadius: 4, padding: "0.4rem 0.7rem", marginTop: 4 }}>
                        <summary style={{ cursor: "pointer", fontWeight: 600, fontSize: 13, display: "flex", alignItems: "center", gap: 6 }}>
                          ...
                          <button
                            ...
                            style={{ marginLeft: "auto", background: "rgba(34,197,94,0.12)", border: "1px solid rgba(34,197,94,0.35)", color: "#4ade80", borderRadius: 4, padding: "1px 8px", cursor: "pointer", fontSize: 11 }}
                        ...
                        <ul style={{ margin: "0.4rem 0 0", paddingLeft: 8, fontSize: 12, listStyle: "none" }}>
                          {s.files.map(ff => {
                            ...
                            return (
                              <li key={ff.name} style={{ opacity: 0.9, display: "flex", alignItems: "center", gap: 6, padding: "3px 4px", background: sel ? "rgba(250,204,21,0.08)" : "transparent", borderRadius: 4 }}>
                                <input
                                  type="checkbox"
                                  checked={sel}
                                ...
                                <button
                                  ...
                                  style={{ background: "rgba(212,175,55,0.15)", border: "1px solid rgba(212,175,55,0.4)", color: "#d4af37", borderRadius: 4, padding: "2px 8px", cursor: "pointer", fontSize: 11 }}
                                >
                                  <i className="fa fa-eye"></i>
                                </button>
                                {ff.name.toLowerCase().endsWith(".pdf") && ff.size > 100000 && (
                                  <button
                                    ...
                                    style={{ background: "rgba(168,85,247,0.15)", border: "1px solid rgba(168,85,247,0.4)", color: "#c084fc", borderRadius: 4, padding: "2px 8px", cursor: "pointer", fontSize: 11 }}
                                  >
                                ...
                                <button
                                  ...
                                  style={{ background: "rgba(148,163,184,0.15)", border: "1px solid rgba(148,163,184,0.4)", color: "#94a3b8", borderRadius: 4, padding: "2px 8px", cursor: "pointer", fontSize: 11 }}
                                >
                                  <i className="fa fa-download"></i>
                                </button>
                                <button
                                  ...
                                  style={{ background: "rgba(239,68,68,0.15)", border: "1px solid rgba(239,68,68,0.4)", color: "#f87171", borderRadius: 4, padding: "2px 8px", cursor: "pointer", fontSize: 11 }}
                                >
                                  <i className="fa fa-trash"></i>
                                </button>
                              </li>
          ...
          >
            <div style={{
              ...
              <a
                ...
                style={{ background: "rgba(148,163,184,0.15)", color: "#e2e8f0", padding: "5px 12px", borderRadius: 4, textDecoration: "none", fontSize: 12 }}
              >
              ...
              <button
                ...
                style={{ background: "rgba(239,68,68,0.2)", color: "#fca5a5", border: "1px solid rgba(239,68,68,0.4)", borderRadius: 4, padding: "5px 12px", cursor: "pointer", fontSize: 12 }}
              >
      ...
      {tasacionModal && (() => {
        ...
        const inpS = { width: "100%", padding: "0.5rem", borderRadius: 4, border: "1px solid rgba(148,163,184,0.3)", background: "#1e293b", color: "#e2e8f0", fontSize: 13 };
        ...
        return (
          <div data-testid="tasacion-modal" onClick={() => setTasacionModal(null)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9998, padding: "3vh 3vw" }}>
            <div onClick={(e) => e.stopPropagation()} style={{ background: "#0f172a", color: "#e2e8f0", borderRadius: 4, width: "min(760px, 96vw)", maxHeight: "94vh", overflow: "auto", border: "1px solid rgba(148,163,184,0.25)", boxShadow: "0 20px 60px rgba(0,0,0,0.6)" }}>
              ...
              <div style={{ padding: "1rem 1.1rem", display: "grid", gap: "0.75rem" }}>
                {m.msg && (
                  <div data-testid="tasacion-msg" style={{ padding: "0.6rem 0.9rem", borderRadius: 4, background: m.msg.startsWith("✅") ? "rgba(34,197,94,0.15)" : "rgba(239,68,68,0.15)", color: m.msg.startsWith("✅") ? "#4ade80" : "#f87171", fontWeight: 600, fontSize: 13 }}>{m.msg}</div>
                )}
                ...
                <div style={{ background: "rgba(30,41,59,0.7)", borderRadius: 4, padding: "0.5rem 0.9rem", fontSize: 12, opacity: 0.85 }}>
                ...
                {m.modalidad === "inmobiliaria" && (
                  <div style={{ background: "rgba(139,92,246,0.08)", border: "1px solid rgba(139,92,246,0.3)", borderRadius: 4, padding: "0.7rem 0.9rem", display: "grid", gap: "0.6rem" }}>
                ...
                {m.modalidad === "broker" && (
                  <div style={{ background: "rgba(212,175,55,0.06)", border: "1px solid rgba(212,175,55,0.3)", borderRadius: 4, padding: "0.7rem 0.9rem", display: "grid", gap: "0.6rem" }}>
                ...
                {!m.archivos.some(a => a.sel && /carta|oferta|aprobaci/i.test(a.nombre)) && (
                  <div data-testid="tasacion-sin-carta" style={{ fontSize: 12, color: "#facc15", background: "rgba(250,204,21,0.08)", border: "1px solid rgba(250,204,21,0.35)", borderRadius: 4, padding: "6px 10px" }}>
                    ⚠️ No hay <b>carta de aprobación</b> seleccionada en los adjuntos — la solicitud debe incluirla.
                  </div>
                )}
                <div style={{ background: "rgba(30,41,59,0.7)", borderRadius: 4, padding: "0.7rem 0.9rem", display: "grid", gap: "0.6rem" }}>
                ...
                <div style={{ background: "rgba(212,175,55,0.08)", border: "1px solid rgba(212,175,55,0.35)", borderRadius: 4, padding: "0.7rem 0.9rem", display: "grid", gap: "0.4rem" }}>
                ...
                <div style={{ background: "rgba(234,88,12,0.08)", border: "1px solid rgba(234,88,12,0.3)", borderRadius: 4, padding: "0.7rem 0.9rem", display: "flex", gap: 8, alignItems: "end", flexWrap: "wrap" }}>
                ...
                {m.archivos.length > 0 && (
                  <div style={{ background: "rgba(30,41,59,0.7)", borderRadius: 4, padding: "0.7rem 0.9rem" }}>
                      ...
                      {m.archivos.map((a, i) => {
                        ...
                        <label key={a.ruta} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12.5, cursor: permitido ? "pointer" : "not-allowed", opacity: permitido ? 1 : 0.45 }}>
                          ...
                          <button type="button" data-testid={`tasacion-adj-ver-${i}`}
                            ...
                            style={{ background: "rgba(212,175,55,0.15)", border: "1px solid #d4af37", color: "#d4af37", borderRadius: 4, padding: "2px 10px", fontSize: 11, fontWeight: 700, cursor: "pointer", flexShrink: 0, opacity: 1 }}>
                ...
                {m.preview && (
                  <div style={{ background: "#fff", borderRadius: 4, padding: "0.9rem", maxHeight: 260, overflow: "auto" }}>
                ...
                <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
                  <button onClick={tasacionPreview} disabled={m.loading} data-testid="btn-tasacion-preview"
                    style={{ padding: "0.55rem 1.1rem", borderRadius: 4, border: "1px solid rgba(148,163,184,0.4)", background: "transparent", color: "#e2e8f0", fontWeight: 700, cursor: "pointer" }}>
                  ...
                  <button onClick={tasacionEnviar} disabled={m.loading || !m.direccion.trim()} data-testid="btn-tasacion-enviar"
                    ...
                    style={{ padding: "0.55rem 1.1rem", borderRadius: 4, border: "none", background: "#ea580c", color: "#fff", fontWeight: 800, cursor: "pointer", opacity: (m.loading || !m.direccion.trim()) ? 0.6 : 1 }}>
        ...
        return (
          <div data-testid="estudio-modal" onClick={() => setEstudioModal(null)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9998, padding: "3vh 3vw" }}>
            <div onClick={(e) => e.stopPropagation()} style={{ background: "#0f172a", color: "#e2e8f0", borderRadius: 4, width: "min(760px, 96vw)", maxHeight: "94vh", overflow: "auto", border: "1px solid rgba(148,163,184,0.25)", boxShadow: "0 20px 60px rgba(0,0,0,0.6)" }}>
              ...
              <div style={{ padding: "1rem 1.1rem", display: "grid", gap: "0.75rem" }}>
                {m.msg && (
                  <div data-testid="estudio-msg" style={{ padding: "0.6rem 0.9rem", borderRadius: 4, background: m.msg.startsWith("✅") ? "rgba(34,197,94,0.15)" : "rgba(239,68,68,0.15)", color: m.msg.startsWith("✅") ? "#4ade80" : "#f87171", fontWeight: 600, fontSize: 13 }}>{m.msg}</div>
                )}
                <div style={{ display: "flex", gap: 8, alignItems: "center", background: "rgba(212,175,55,0.06)", border: "1px solid rgba(212,175,55,0.25)", borderRadius: 4, padding: "0.5rem 0.9rem", flexWrap: "wrap" }}>
                  ...
                  <button data-testid="estudio-plantilla-guardar" onClick={guardarPlantillaEstudio}
                    style={{ background: "rgba(212,175,55,0.15)", border: "1px solid rgba(212,175,55,0.5)", color: "#d4af37", borderRadius: 4, padding: "5px 12px", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>
                    💾 Guardar plantilla
                  </button>
                ...
                <button data-testid="estudio-enriquecer" onClick={() => enriquecerCarpeta(m.folder, "estudio")}
                  ...
                  style={{ background: "rgba(6,182,212,0.12)", border: "1.5px solid #06b6d4", color: "#22d3ee", borderRadius: 4, padding: "0.55rem 1rem", fontSize: 12.5, fontWeight: 800, cursor: "pointer", textAlign: "left" }}>
                ...
                <div style={{ background: "rgba(30,41,59,0.5)", border: "1px solid rgba(148,163,184,0.2)", borderRadius: 4, padding: "0.6rem 0.9rem" }}>
                  ...
                  <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 6 }}>
                    {m.folder.source_email && !String(m.cc || "").toLowerCase().includes(String(m.folder.source_email).toLowerCase()) && (
                      <button data-testid="cc-add-cliente" onClick={() => set("cc", [m.cc, m.folder.source_email].filter(Boolean).join(", "))}
                        style={{ background: "rgba(34,197,94,0.12)", border: "1px solid rgba(34,197,94,0.4)", color: "#4ade80", borderRadius: 4, padding: "3px 10px", fontSize: 11.5, cursor: "pointer" }}>
                    ...
                    {brokers.flatMap(b => (b.emails || []).map(em => ({ nombre: b.nombre, em })))
                      ...
                      .map((x, i) => (
                        <button key={i} data-testid={`cc-add-broker-${i}`} onClick={() => set("cc", [m.cc, x.em].filter(Boolean).join(", "))}
                          style={{ background: "rgba(212,175,55,0.12)", border: "1px solid rgba(212,175,55,0.4)", color: "#d4af37", borderRadius: 4, padding: "3px 10px", fontSize: 11.5, cursor: "pointer" }}>
                          + {x.nombre} ({x.em})
                        </button>
                      ))}
                  </div>
                ...
                {m.archivos.length > 0 && (() => {
                  ...
                  {docsEstudio.length > 0 && (
                    <div data-testid="estudio-docs-recibidos" style={{ background: "rgba(20,184,166,0.08)", border: "1.5px dashed #14b8a6", borderRadius: 4, padding: "0.7rem 0.9rem" }}>
                      ...
                      <button data-testid="btn-estudio-etapa2" onClick={() => enviarEtapa2(m)} disabled={m.loading}
                        ...
                        style={{ background: "#0d9488", border: "none", color: "#fff", borderRadius: 4, padding: "0.5rem 1rem", fontSize: 12.5, fontWeight: 800, cursor: "pointer", marginBottom: 8 }}>
                        ...
                        {docsEstudio.map((a, i) => (
                          <div key={a.ruta} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12.5 }}>
                            ...
                            <button type="button" data-testid={`estudio-doc-ver-${i}`}
                              ...
                              style={{ background: "rgba(212,175,55,0.15)", border: "1px solid #d4af37", color: "#d4af37", borderRadius: 4, padding: "2px 10px", fontSize: 11, fontWeight: 700, cursor: "pointer", flexShrink: 0 }}>
                              <i className="fa fa-eye" /> Ver
                            </button>
                          </div>
                        ))}
                ...
                <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
                  ...
                  <button onClick={estudioEnviar} disabled={m.loading} data-testid="btn-estudio-enviar"
                    style={{ padding: "0.55rem 1.1rem", borderRadius: 4, border: "none", background: "#0d9488", color: "#fff", fontWeight: 800, cursor: "pointer", opacity: m.loading ? 0.6 : 1 }}>
                    <i className={`fa ${m.loading ? "fa-spinner fa-spin" : "fa-paper-plane"}`} /> Enviar solicitud
                  </button>
                </div>
              </div>
      ...
      {pedirModal && (() => {
        ...
        return (
          <div data-testid="pedir-modal" onClick={() => setPedirModal(null)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9998, padding: "3vh 3vw" }}>
            <div onClick={(e) => e.stopPropagation()} style={{ background: "#0f172a", color: "#e2e8f0", borderRadius: 4, width: "min(680px, 96vw)", maxHeight: "94vh", overflow: "auto", border: "1px solid rgba(148,163,184,0.25)", boxShadow: "0 20px 60px rgba(0,0,0,0.6)" }}>
              <div style={{ padding: "0.9rem 1.1rem", borderBottom: "1px solid rgba(148,163,184,0.2)", display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <div style={{ padding: "1rem 1.1rem", display: "grid", gap: "0.75rem" }}>
                {m.msg && (
                  <div data-testid="pedir-msg" style={{ padding: "0.6rem 0.9rem", borderRadius: 4, background: m.msg.startsWith("✅") ? "rgba(34,197,94,0.15)" : "rgba(239,68,68,0.15)", color: m.msg.startsWith("✅") ? "#4ade80" : "#f87171", fontWeight: 600, fontSize: 13 }}>{m.msg}</div>
                )}
                ...
                {m.preview && (
                  <div style={{ background: "#fff", borderRadius: 4, padding: "0.9rem", maxHeight: 240, overflow: "auto" }}>
                    <div dangerouslySetInnerHTML={{ __html: m.preview.body }} />
                  </div>
                )}
                <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
                  ...
                  <button onClick={pedirEnviar} disabled={m.loading || !m.destinatario.includes("@")} data-testid="btn-pedir-enviar"
                    style={{ padding: "0.55rem 1.1rem", borderRadius: 4, border: "none", background: "#dc2626", color: "#fff", fontWeight: 800, cursor: "pointer", opacity: (m.loading || !m.destinatario.includes("@")) ? 0.6 : 1 }}>
        ...
        return (
          <div data-testid="escritura-modal" onClick={() => setEscrituraModal(null)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9998, padding: "3vh 3vw" }}>
            <div onClick={(e) => e.stopPropagation()} style={{ background: "#0f172a", color: "#e2e8f0", borderRadius: 4, width: "min(760px, 96vw)", maxHeight: "94vh", overflow: "auto", border: "1px solid rgba(148,163,184,0.25)", boxShadow: "0 20px 60px rgba(0,0,0,0.6)" }}>
              ...
              <div style={{ padding: "1rem 1.1rem", display: "grid", gap: "0.75rem" }}>
                {m.msg && (
                  <div data-testid="escritura-msg" style={{ padding: "0.6rem 0.9rem", borderRadius: 4, background: m.msg.startsWith("✅") ? "rgba(34,197,94,0.15)" : "rgba(239,68,68,0.15)", color: m.msg.startsWith("✅") ? "#4ade80" : "#f87171", fontWeight: 600, fontSize: 13 }}>{m.msg}</div>
                )}
                ...
                {notariaSel && !notariaSel.email && (
                  <div style={{ display: "flex", gap: 6, alignItems: "center", background: "rgba(250,204,21,0.08)", border: "1px solid rgba(250,204,21,0.3)", borderRadius: 4, padding: "0.5rem 0.8rem", fontSize: 12 }}>
                ...
                {m.addNotaria ? (
                  <div style={{ background: "rgba(236,72,153,0.06)", border: "1px dashed rgba(236,72,153,0.4)", borderRadius: 4, padding: "0.7rem 0.9rem", display: "grid", gap: "0.5rem" }}>
                  ...
                  <button onClick={() => setEscrituraModal(prev => ({ ...prev, addNotaria: true }))} data-testid="btn-add-notaria" style={{ justifySelf: "start", background: "transparent", border: "1px dashed rgba(236,72,153,0.5)", color: "#f9a8d4", borderRadius: 4, padding: "0.3rem 0.7rem", fontSize: 11.5, cursor: "pointer" }}>
                ...
                <div style={{ fontSize: 11.5, opacity: 0.75, background: "rgba(34,197,94,0.06)", border: "1px solid rgba(34,197,94,0.25)", borderRadius: 4, padding: "0.5rem 0.8rem", lineHeight: 1.6 }}>
                ...
                {m.preview && (
                  <div style={{ background: "#fff", borderRadius: 4, padding: "0.9rem", maxHeight: 280, overflow: "auto" }}>
                    <div dangerouslySetInnerHTML={{ __html: m.preview.body }} />
                  </div>
                )}
                <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
                  ...
                  <button onClick={escrituraEnviar} disabled={m.loading || !m.email_cliente.includes("@") || !m.fecha || !m.notaria_id} data-testid="btn-escritura-enviar"
                    ...
                    style={{ padding: "0.55rem 1.1rem", borderRadius: 4, border: "none", background: "#db2777", color: "#fff", fontWeight: 800, cursor: "pointer", opacity: (m.loading || !m.email_cliente.includes("@") || !m.fecha || !m.notaria_id) ? 0.6 : 1 }}>
      ...
      {emailModal && (() => {
        ...
        return (
          <div data-testid="email-modal" onClick={closeEmailModal} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9998, padding: "3vh 3vw" }}>
            <div onClick={(e) => e.stopPropagation()} style={{ background: "#0f172a", color: "#e2e8f0", borderRadius: 4, width: "min(760px, 96vw)", maxHeight: "94vh", overflow: "auto", border: "1px solid rgba(148,163,184,0.25)", boxShadow: "0 20px 60px rgba(0,0,0,0.6)" }}>
              <div style={{ padding: "0.9rem 1.1rem", borderBottom: "1px solid rgba(148,163,184,0.2)", display: "flex", alignItems: "center", gap: 10 }}>
                <i className="fa fa-paper-plane" style={{ color: "#93c5fd" }} />
              ...
              <div style={{ padding: "1rem 1.1rem", display: "grid", gap: "0.75rem" }}>
                <div style={{ background: "rgba(30,41,59,0.7)", borderRadius: 4, padding: "0.7rem 0.9rem", fontSize: 13, lineHeight: 1.5 }}>
                ...
                <label style={{ fontSize: 12 }}>
                  ...
                  <input
                    ...
                    style={{ width: "100%", padding: "0.5rem", borderRadius: 4, border: "1px solid rgba(148,163,184,0.3)", background: "#1e293b", color: "#e2e8f0", fontSize: 13, cursor: "not-allowed", opacity: 0.85 }}
                  />
                  <div style={{ marginTop: 6, padding: "6px 10px", borderRadius: 4, background: "rgba(239,68,68,0.12)", border: "1px solid rgba(239,68,68,0.4)", color: "#fca5a5", fontSize: 11, fontWeight: 500 }}>
                ...
                <label style={{ fontSize: 12 }}>
                  ...
                  <select
                    ...
                    style={{ width: "100%", padding: "0.5rem", borderRadius: 4, border: "1px solid rgba(148,163,184,0.3)", background: "#1e293b", color: "#e2e8f0", fontSize: 13 }}>
                ...
                <label style={{ fontSize: 12 }}>
                  ...
                  <div style={{ padding: "0.45rem 0.6rem", borderRadius: 4, background: "rgba(239,68,68,0.08)", border: "1px dashed rgba(239,68,68,0.4)", fontSize: 12, color: "#fca5a5", marginBottom: 6 }} data-testid="email-subject-prefijo">
                    🔒 {prefijoAsunto}
                  </div>
                  <input
                    type="text"
                ...
                <label style={{ fontSize: 12 }}>
                  ...
                  <textarea
                    ...
                    style={{ width: "100%", padding: "0.5rem", borderRadius: 4, border: "1px solid rgba(148,163,184,0.3)", background: "#1e293b", color: "#e2e8f0", fontSize: 13, resize: "vertical" }}
                  />
                </label>

                <div style={{ background: "rgba(30,41,59,0.5)", borderRadius: 4, padding: "0.6rem 0.9rem", fontSize: 12, display: "grid", gap: 6 }}>
              ...
              {emailModal.preview && (
                <div style={{ padding: "0.8rem 1.1rem", borderTop: "1px solid rgba(148,163,184,0.2)" }}>
                  ...
                  {(emailModal.preview.missing_docs || []).length > 0 && (
                    <div data-testid="email-missing-docs-warning" style={{ marginBottom: 8, padding: "8px 12px", borderRadius: 4, background: "rgba(239,68,68,0.12)", border: "1px solid rgba(239,68,68,0.5)", color: "#fca5a5", fontSize: 12 }}>
                  ...
                  {emailModal.editBody != null ? (
                    <div>
                      <textarea value={emailModal.editBody} data-testid="email-body-editor"
                        ...
                        rows={12}
                        style={{ width: "100%", padding: "0.6rem 0.8rem", borderRadius: 4, border: "1px solid rgba(250,204,21,0.5)", background: "#1e293b", color: "#e2e8f0", fontSize: 12, fontFamily: "monospace", resize: "vertical" }} />
                      <div style={{ marginTop: 6, background: "#fff", color: "#111", borderRadius: 4, padding: "0.6rem 0.8rem", maxHeight: 200, overflow: "auto", border: "1px solid rgba(148,163,184,0.3)" }}
                      ...
                      <button onClick={() => setEmailModal({ ...emailModal, editBody: null })} data-testid="email-body-reset"
                        style={{ marginTop: 6, background: "transparent", border: "1px solid rgba(148,163,184,0.3)", color: "#94a3b8", borderRadius: 4, padding: "4px 10px", cursor: "pointer", fontSize: 12 }}>
                        <i className="fa fa-undo" /> Volver al cuerpo automático
                      </button>
                    </div>
                  ) : (
                    <div>
                      <div style={{ background: "#fff", color: "#111", borderRadius: 4, padding: "0.6rem 0.8rem", maxHeight: 320, overflow: "auto", border: "1px solid rgba(148,163,184,0.3)" }}
                      ...
                      <button onClick={() => setEmailModal({ ...emailModal, editBody: emailModal.preview.body_html || emailModal.preview.body || "" })} data-testid="email-body-edit-btn"
                        style={{ marginTop: 6, background: "rgba(250,204,21,0.15)", border: "1px solid rgba(250,204,21,0.5)", color: "#facc15", borderRadius: 4, padding: "4px 10px", cursor: "pointer", fontSize: 12, fontWeight: 600 }}>
              ...
              <div style={{ padding: "0.8rem 1.1rem", borderTop: "1px solid rgba(148,163,184,0.2)", display: "flex", gap: 8, justifyContent: "flex-end" }}>
                <button onClick={closeEmailModal} disabled={emailModal.sending} style={{ background: "transparent", border: "1px solid rgba(148,163,184,0.3)", color: "#94a3b8", borderRadius: 4, padding: "6px 14px", cursor: "pointer", fontSize: 13 }}>Cancelar</button>
                {!emailModal.preview ? (
                  <button onClick={previewClientEmail} disabled={emailModal.sending || !emailModal.to} data-testid="btn-email-preview"
                    style={{ background: !emailModal.to ? "rgba(148,163,184,0.15)" : "rgba(250,204,21,0.2)", border: `1px solid ${!emailModal.to ? "rgba(148,163,184,0.3)" : "rgba(250,204,21,0.6)"}`, color: !emailModal.to ? "#94a3b8" : "#facc15", borderRadius: 4, padding: "6px 14px", cursor: (emailModal.sending || !emailModal.to) ? "not-allowed" : "pointer", fontSize: 13, fontWeight: 600 }}
                  ...
                  <>
                    <button onClick={() => setEmailModal({ ...emailModal, preview: null })} disabled={emailModal.sending}
                      style={{ background: "transparent", border: "1px solid rgba(148,163,184,0.3)", color: "#94a3b8", borderRadius: 4, padding: "6px 14px", cursor: "pointer", fontSize: 13 }}>
                      <i className="fa fa-pencil" /> Editar
                    </button>
                    <button onClick={async () => {
                      ...
                      style={{ background: "#ec4899", border: "1px solid #be185d", color: "#fff", borderRadius: 4, padding: "6px 14px", cursor: "pointer", fontSize: 13, fontWeight: 700 }}>
                      ...
                      return (
                        <button onClick={confirmSendClientEmail} disabled={emailModal.sending || bloqueado} data-testid="btn-email-send" title={title}
                          style={{
                            ...
                            color: "#fff", borderRadius: 4, padding: "6px 18px",
      ...
      {historialModal && (
        <div data-testid="historial-modal" onClick={() => setHistorialModal(null)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9998, padding: "3vh 3vw" }}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: "#0f172a", color: "#e2e8f0", borderRadius: 4, width: "min(640px, 96vw)", maxHeight: "90vh", overflow: "auto", border: "1px solid rgba(212,175,55,0.4)", boxShadow: "0 20px 60px rgba(0,0,0,0.6)" }}>
      ...
      {missingDocsModal && (() => {
        ...
        return (
          <div data-testid="missing-docs-modal" onClick={closeMissingDocsModal} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9998, padding: "3vh 3vw" }}>
            <div onClick={(e) => e.stopPropagation()} style={{ background: "#0f172a", color: "#e2e8f0", borderRadius: 4, width: "min(760px, 96vw)", maxHeight: "94vh", overflow: "auto", border: "1px solid rgba(239,68,68,0.5)", boxShadow: "0 20px 60px rgba(0,0,0,0.6)" }}>
              <div style={{ padding: "0.9rem 1.1rem", borderBottom: "1px solid rgba(148,163,184,0.2)", display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <div style={{ padding: "1rem 1.1rem", display: "grid", gap: "0.75rem" }}>
                {p && (
                  <>
                    <div style={{ background: "rgba(239,68,68,0.1)", borderRadius: 4, padding: "0.7rem 0.9rem", fontSize: 13 }}>
                    ...
                    <label style={{ fontSize: 12 }}><b style={{ display: "block", marginBottom: 4 }}>Destinatario *</b>
                      <input type="email" value={missingDocsModal.to} onChange={(e) => setMissingDocsModal({ ...missingDocsModal, to: e.target.value })} onBlur={refreshMissingDocsPreview} data-testid="missing-to" style={{ width: "100%", padding: "0.5rem", borderRadius: 4, border: "1px solid rgba(148,163,184,0.3)", background: "#1e293b", color: "#e2e8f0", fontSize: 13 }} />
                    </label>
                    <label style={{ fontSize: 12 }}><b style={{ display: "block", marginBottom: 4 }}>Mensaje adicional (opcional)</b>
                      <textarea value={missingDocsModal.extra} onChange={(e) => setMissingDocsModal({ ...missingDocsModal, extra: e.target.value })} onBlur={refreshMissingDocsPreview} rows={2} data-testid="missing-extra" style={{ width: "100%", padding: "0.5rem", borderRadius: 4, border: "1px solid rgba(148,163,184,0.3)", background: "#1e293b", color: "#e2e8f0", fontSize: 13 }} />
                    </label>
                    <div>
                      ...
                      <div style={{ background: "#fff", color: "#111", borderRadius: 4, padding: "0.6rem 0.8rem", maxHeight: 280, overflow: "auto", border: "1px solid rgba(148,163,184,0.3)" }}
              ...
              <div style={{ padding: "0.8rem 1.1rem", borderTop: "1px solid rgba(148,163,184,0.2)", display: "flex", gap: 8, justifyContent: "flex-end" }}>
                <button onClick={closeMissingDocsModal} disabled={missingDocsModal.sending} style={{ background: "transparent", border: "1px solid rgba(148,163,184,0.3)", color: "#94a3b8", borderRadius: 4, padding: "6px 14px", cursor: "pointer", fontSize: 13 }}>Cancelar</button>
                <button onClick={confirmSendMissingDocs} disabled={missingDocsModal.sending || !p} data-testid="btn-missing-send"
                  style={{ background: "#16a34a", border: "1px solid #15803d", color: "#fff", borderRadius: 4, padding: "6px 18px", cursor: "pointer", fontSize: 13, fontWeight: 700, boxShadow: "0 2px 8px rgba(22,163,74,0.4)" }}>
      ...
      {forzarModal && (() => {
        const m = forzarModal;
        const inpF = { width: "100%", padding: "0.55rem", borderRadius: 4, border: "1px solid rgba(148,163,184,0.3)", background: "#1e293b", color: "#e2e8f0", fontSize: 13 };
        return (
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.65)", zIndex: 9998, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }} onClick={() => !m.forzando && setForzarModal(null)}>
            <div data-testid="forzar-modal" onClick={e => e.stopPropagation()} style={{ background: "#0f172a", border: "1px solid rgba(148,163,184,0.25)", borderRadius: 4, padding: "1.4rem", width: "min(640px, 96vw)", maxHeight: "90vh", overflow: "auto", display: "grid", gap: 10 }}>
              ...
              {m.sug && !m.buscando && (
                <div data-testid="forzar-sugerencias" style={{ display: "grid", gap: 8 }}>
                  {(m.sug.carpetas || []).length > 0 && (
                    <div style={{ background: "rgba(212,175,55,0.06)", border: "1px dashed rgba(212,175,55,0.4)", borderRadius: 4, padding: "0.6rem 0.8rem" }}>
                  ...
                  {(m.sug.correos || []).length > 0 && (
                    <div style={{ background: "rgba(6,182,212,0.06)", border: "1px dashed rgba(6,182,212,0.4)", borderRadius: 4, padding: "0.6rem 0.8rem" }}>
                  ...
                  {(m.sug.cola || []).length > 0 && (
                    <div style={{ background: "rgba(139,92,246,0.06)", border: "1px dashed rgba(139,92,246,0.4)", borderRadius: 4, padding: "0.6rem 0.8rem" }}>
              ...
              <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
                <button onClick={() => setForzarModal(null)} disabled={m.forzando} style={{ background: "transparent", border: "1px solid rgba(148,163,184,0.3)", color: "#94a3b8", borderRadius: 4, padding: "0.55rem 1rem", fontSize: 12.5, cursor: "pointer" }}>Cerrar</button>
                <button data-testid="forzar-ejecutar" onClick={ejecutarForzar} disabled={m.forzando || (!m.nombre.trim() && !m.rut.trim())}
                  style={{ background: "#f59e0b", border: "none", color: "#0f172a", borderRadius: 4, padding: "0.55rem 1.3rem", fontSize: 12.5, fontWeight: 800, cursor: "pointer" }}>
      ...
      {reparosModal && (() => {
        ...
        return (
          <div data-testid="reparos-modal" onClick={() => setReparosModal(null)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9998, padding: "3vh 3vw" }}>
            <div onClick={(e) => e.stopPropagation()} style={{ background: "#0f172a", color: "#e2e8f0", borderRadius: 4, width: "min(720px, 96vw)", maxHeight: "94vh", overflow: "auto", border: "1px solid rgba(148,163,184,0.25)", boxShadow: "0 20px 60px rgba(0,0,0,0.6)" }}>
              <div style={{ padding: "0.9rem 1.1rem", borderBottom: "1px solid rgba(148,163,184,0.2)", display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <div style={{ padding: "1rem 1.1rem", display: "grid", gap: "0.75rem" }}>
                {m.msg && (
                  <div data-testid="reparos-msg" style={{ padding: "0.6rem 0.9rem", borderRadius: 4, background: m.msg.startsWith("Error") ? "rgba(239,68,68,0.15)" : "rgba(34,197,94,0.15)", color: m.msg.startsWith("Error") ? "#f87171" : "#4ade80", fontWeight: 600, fontSize: 13 }}>{m.msg}</div>
                )}
                {m.loading ? (
                  <div style={{ textAlign: "center", padding: "1.5rem" }}><i className="fa fa-spinner fa-spin" /> Cargando reparos…</div>
                ) : (
                  <>
                    <div style={{ fontSize: 12, color: "#94a3b8", background: "rgba(148,163,184,0.07)", border: "1px solid rgba(148,163,184,0.15)", borderRadius: 4, padding: "0.6rem 0.9rem", display: "grid", gap: 3 }}>
                    ...
                    {items.length === 0 ? (
                      ...
                      <div style={{ display: "grid", gap: 6 }}>
                        {items.map((it) => (
                          <label key={it.n} data-testid={`reparo-item-${it.n}`} style={{ display: "flex", alignItems: "flex-start", gap: 10, background: it.satisfecho ? "rgba(34,197,94,0.08)" : "rgba(239,68,68,0.06)", border: `1px solid ${it.satisfecho ? "rgba(34,197,94,0.35)" : "rgba(239,68,68,0.25)"}`, borderRadius: 4, padding: "0.6rem 0.9rem", cursor: "pointer" }}>
                    ...
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap", justifyContent: "flex-end", borderTop: "1px solid rgba(148,163,184,0.15)", paddingTop: "0.8rem" }}>
                      <button onClick={scanReparos} disabled={m.scanning} data-testid="btn-reparos-scan"
                        style={{ background: "rgba(212,175,55,0.15)", border: "1px solid #d4af37", color: "#d4af37", borderRadius: 4, padding: "8px 14px", cursor: "pointer", fontSize: 13, fontWeight: 700 }}>
                        <i className={`fa ${m.scanning ? "fa-spinner fa-spin" : "fa-refresh"}`} /> {m.scanning ? "Revisando hilo…" : "Buscar reparos ahora"}
                      </button>
                      {satisfecho ? (
                        <div data-testid="reparos-satisfecho-badge" style={{ background: "rgba(34,197,94,0.15)", border: "1px solid #22c55e", color: "#4ade80", borderRadius: 4, padding: "8px 14px", fontSize: 13, fontWeight: 700 }}>
                        ...
                        <button onClick={declararReparos} disabled={m.declarando || items.length === 0 || pendientes > 0} data-testid="btn-reparos-declarar"
                          ...
                          style={{ background: (m.declarando || items.length === 0 || pendientes > 0) ? "#475569" : "#16a34a", border: "none", color: "#fff", borderRadius: 4, padding: "8px 16px", cursor: (items.length === 0 || pendientes > 0) ? "not-allowed" : "pointer", fontSize: 13, fontWeight: 700 }}>
