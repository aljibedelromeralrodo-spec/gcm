...
export function StatBox({ label, value, warn }) {
  return (
    <div style={{ padding: "0.6rem", borderRadius: "10px", background: COLORS.card, border: `1px solid ${COLORS.border}`, textAlign: "center" }}>
...
export function DashCard({ label, value, icon, color }) {
  return (
    <div style={{ padding: "1rem", borderRadius: "14px", background: COLORS.card, border: `1px solid ${COLORS.border}`, textAlign: "center" }}>
...
export function EvalBadge({ label, result, razones }) {
  ...
  return (
    <div style={{ padding: "0.6rem", borderRadius: "10px", background: bgColor, border: `1px solid ${borderColorEval}`, textAlign: "center" }}>
      <div style={{ fontSize: "0.7rem", color: COLORS.textMuted }}>{label}</div>
...
export function ScoreHistory({ clientName, apiUrl }) {
  ...
  return (
    <div data-testid="score-history" style={{ marginTop: "1rem", padding: "1rem", borderRadius: "14px", background: COLORS.card, border: `1px solid ${COLORS.border}` }}>
      ...
      {loading ? <div style={{ color: COLORS.textMuted, fontSize: "0.8rem" }}>Cargando...</div> : (
        <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem" }}>
          {history.map((h, i) => {
            ...
            return (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: "0.5rem", padding: "0.4rem 0.6rem", borderRadius: "8px", background: i === 0 ? "rgba(108,92,231,0.06)" : "transparent" }}>
