...
export function PredICResult({ result, onExportPDF, form }) {
  ...
  if (!anyViable) {
    return (
      <div data-testid="predic-result" style={{ marginTop: "1.25rem", borderRadius: "2px", overflow: "hidden", border: "2px solid rgba(225,112,85,0.4)", background: "rgba(225,112,85,0.08)" }}>
        {/* Status */}
        ...
        <div style={{ padding: "1.25rem" }}>
          ...
          {result.razones?.length > 0 && (
            <div data-testid="predic-result-reasons" style={{ padding: "1rem", borderRadius: "2px", background: "rgba(225,112,85,0.1)", border: "1px solid rgba(225,112,85,0.2)", marginBottom: "1rem" }}>
          ...
          {result.sugerencias_optimizacion?.length > 0 && (
            <div data-testid="predic-result-sugerencias" style={{ padding: "1rem", borderRadius: "2px", background: "rgba(108,92,231,0.08)", border: "1px solid rgba(108,92,231,0.2)", marginBottom: "1rem" }}>
  ...
  return (
    <div data-testid="predic-result" style={{ marginTop: "1.25rem", borderRadius: "2px", overflow: "hidden", border: `2px solid ${borderColor}`, background: bg }}>
      {/* Status banner */}
      ...
      <div style={{ padding: "1.25rem" }}>
        ...
        {result.valor_propiedad_uf > 0 && (
          <div data-testid="predic-result-propiedad" style={{ textAlign: "center", marginBottom: "0.75rem", padding: "0.7rem", borderRadius: "2px", background: "rgba(212,175,55,0.08)", border: `2px solid ${COLORS.gold}` }}>
        ...
        {result.credito_solicitado_uf > 0 && (
          <div style={{ textAlign: "center", marginBottom: "0.75rem", padding: "0.7rem", borderRadius: "2px", background: "rgba(108,92,231,0.06)", border: `1px solid ${COLORS.border}` }}>
        ...
        <div data-testid="predic-result-max" style={{ textAlign: "center", marginBottom: "1rem", padding: "1rem", borderRadius: "2px", background: "rgba(108,92,231,0.1)", border: `1px solid ${COLORS.border}` }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: result.dividendo_alternativo_clp > 0 ? "1fr 1fr" : "1fr", gap: "0.5rem", marginBottom: "1rem" }}>
          <div data-testid="predic-result-dividendo-principal" style={{ textAlign: "center", padding: "0.8rem", borderRadius: "2px", background: "rgba(108,92,231,0.08)", border: `2px solid ${COLORS.accent}` }}>
          ...
          {result.dividendo_alternativo_clp > 0 && (
            <div data-testid="predic-result-dividendo-alternativo" style={{ textAlign: "center", padding: "0.8rem", borderRadius: "2px", background: COLORS.card, border: `1px solid ${COLORS.border}` }}>
        ...
        {result.seguros && (
          <div data-testid="predic-seguros-section" style={{ marginBottom: "1rem" }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.5rem", marginBottom: "0.4rem" }}>
              <div data-testid="seguro-desgravamen-box" style={{ textAlign: "center", padding: "0.6rem", borderRadius: "2px", background: "rgba(108,92,231,0.06)", border: `1px solid ${COLORS.border}` }}>
              ...
              <div data-testid="seguro-incendio-box" style={{ textAlign: "center", padding: "0.6rem", borderRadius: "2px", background: "rgba(108,92,231,0.06)", border: `1px solid ${COLORS.border}` }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: result.seguros.dividendo_final_alt > 0 ? "1fr 1fr" : "1fr", gap: "0.5rem" }}>
              <div data-testid="total-dividendo-final-box" style={{ textAlign: "center", padding: "0.7rem", borderRadius: "2px", background: "linear-gradient(135deg, rgba(108,92,231,0.15), rgba(162,155,254,0.1))", border: `2px solid ${COLORS.accent}` }}>
              ...
              {result.seguros.dividendo_final_alt > 0 && (
                <div data-testid="total-dividendo-final-alt-box" style={{ textAlign: "center", padding: "0.7rem", borderRadius: "2px", background: COLORS.card, border: `1px solid ${COLORS.border}` }}>
        ...
        {result.tipo_deudor && (
          <div style={{ display: "flex", gap: "0.5rem", marginBottom: "0.75rem", flexWrap: "wrap" }}>
            <span style={{ padding: "3px 8px", borderRadius: "2px", fontSize: "0.72rem", background: "rgba(108,92,231,0.12)", color: COLORS.accentLight }}>{result.tipo_deudor}</span>
            {result.edad_plazo > 0 && <span style={{ padding: "3px 8px", borderRadius: "2px", fontSize: "0.72rem", background: (result.edad_final_referencia || result.edad_plazo) > 80 ? "rgba(225,112,85,0.15)" : "rgba(0,184,148,0.12)", color: (result.edad_final_referencia || result.edad_plazo) > 80 ? COLORS.red : COLORS.green }}>Edad+Plazo: {result.edad_final_referencia || result.edad_plazo}{result.edad_plazo_codeudor > 0 ? ` (Titular: ${result.edad_plazo}, Codeudor: ${result.edad_plazo_codeudor})` : ""}</span>}
            {result.plazo_anos > 0 && <span data-testid="predic-result-plazo" style={{ padding: "3px 8px", borderRadius: "2px", fontSize: "0.72rem", background: result.plazo_en_rango_preferido ? "rgba(0,184,148,0.12)" : "rgba(243,156,18,0.12)", color: result.plazo_en_rango_preferido ? COLORS.green : COLORS.orange }}>
            ...
            {result.plazo_recomendado > 0 && result.plazo_anos !== result.plazo_recomendado && (
              <span style={{ padding: "3px 8px", borderRadius: "2px", fontSize: "0.72rem", background: "rgba(108,92,231,0.12)", color: COLORS.accentLight }}>
                Recomendado: {result.plazo_recomendado} anos
              </span>
            )}
            {result.tasa_aplicada > 0 && <span style={{ padding: "3px 8px", borderRadius: "2px", fontSize: "0.72rem", background: "rgba(212,175,55,0.12)", color: COLORS.gold }}>Tasa: {result.tasa_aplicada}%</span>}
          </div>
        )}

        {/* Mesa Learning Insight */}
        {result.mesa_learning && result.mesa_learning.source === "mesa_learning" && (
          <div data-testid="predic-mesa-insight" style={{ padding: "0.75rem", borderRadius: "2px", background: "rgba(108,92,231,0.08)", border: `1px solid ${COLORS.border}`, marginBottom: "0.75rem" }}>
            ...
            <div style={{ display: "flex", gap: "0.5rem", marginTop: "0.4rem", flexWrap: "wrap" }}>
              <span style={{ padding: "2px 6px", borderRadius: "2px", fontSize: "0.68rem", background: "rgba(0,184,148,0.12)", color: COLORS.green }}>
                Plazo promedio aprobado: {result.mesa_learning.plazo_promedio_aprobado} anos
              </span>
              <span style={{ padding: "2px 6px", borderRadius: "2px", fontSize: "0.68rem", background: "rgba(108,92,231,0.12)", color: COLORS.accentLight }}>
        ...
        {(result.castigo_renta_variable > 0 || result.castigo_renta_honorarios > 0) && (
          <div data-testid="predic-result-castigos" style={{ padding: "0.75rem", borderRadius: "2px", background: "rgba(243,156,18,0.08)", border: "1px solid rgba(243,156,18,0.2)", marginBottom: "0.75rem" }}>
        ...
        <button data-testid="predic-export-pdf" onClick={onExportPDF}
          style={{ marginTop: "1rem", width: "100%", padding: "0.75rem", borderRadius: "2px", border: `1px solid ${COLORS.gold}`, background: `linear-gradient(135deg, rgba(212,175,55,0.15), rgba(212,175,55,0.05))`, color: COLORS.gold, fontWeight: 700, fontSize: "0.9rem", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: "0.5rem" }}>
