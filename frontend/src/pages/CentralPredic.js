...
function PredICLogin({ form, setForm, onLogin, error, loading }) {
  return (
    <div style={{ minHeight: "100vh", background: `linear-gradient(135deg, ${COLORS.bg} 0%, #1a1a4e 50%, #0a0e27 100%)`, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Segoe UI', sans-serif" }}>
      <div style={{ width: "380px", padding: "2.5rem", borderRadius: "4px", background: COLORS.card, border: `1px solid ${COLORS.border}`, boxShadow: "0 20px 60px rgba(0,0,0,0.5)" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
          <input data-testid="predic-login-user" placeholder="Usuario" value={form.usuario}
            ...
            style={{ padding: "0.85rem 1rem", borderRadius: "4px", border: `1px solid ${COLORS.border}`, background: "rgba(255,255,255,0.05)", color: COLORS.text, fontSize: "1rem", outline: "none" }}
          />
          ...
          <button data-testid="predic-login-btn" onClick={onLogin} disabled={loading}
            style={{ padding: "0.9rem", borderRadius: "4px", border: "none", background: `linear-gradient(135deg, ${COLORS.accent}, #4834d4)`, color: "#fff", fontSize: "1rem", fontWeight: 700, cursor: loading ? "wait" : "pointer", letterSpacing: "1px" }}>
...
function PredICApp({ auth, onLogout }) {
  ...
  const S = { input: { width: "100%", padding: "0.75rem 1rem", borderRadius: "4px", border: `1px solid ${COLORS.border}`, background: "rgba(255,255,255,0.06)", color: COLORS.text, fontSize: "0.95rem", outline: "none", boxSizing: "border-box" } };
    ...
    <div style={{ minHeight: "100vh", background: `linear-gradient(135deg, ${COLORS.bg} 0%, #1a1a4e 50%, #0a0e27 100%)`, fontFamily: "'Segoe UI', sans-serif", color: COLORS.text }}>
        ...
        <div style={{ display: "flex", gap: "0.5rem", alignItems: "center" }}>
          {[["predictor", "fa-calculator", "Predictor"], ["dashboard", "fa-chart-bar", "Mi Dashboard"], ["config", "fa-cog", "Tasas"]].map(([v, icon, label]) => (
            <button key={v} data-testid={`predic-tab-${v}`} onClick={() => { setView(v); if (v === "dashboard") loadDashboard(); }}
              style={{ padding: "0.4rem 0.8rem", borderRadius: "4px", border: `1px solid ${view === v ? COLORS.accent : COLORS.border}`, background: view === v ? "rgba(108,92,231,0.15)" : "transparent", color: view === v ? COLORS.accentLight : COLORS.textMuted, fontSize: "0.78rem", fontWeight: 600, cursor: "pointer" }}>
              <i className={`fa ${icon}`} style={{ marginRight: "0.3rem" }}></i>{label}
            </button>
          ))}
          <button data-testid="predic-logout" onClick={onLogout} style={{ padding: "0.4rem 1rem", borderRadius: "4px", border: `1px solid ${COLORS.border}`, background: "transparent", color: COLORS.textMuted, cursor: "pointer", fontSize: "0.8rem" }}>
            Salir
          </button>
        </div>
      </div>
          ...
          {!dashData ? (
            ...
            <>
              ...
              {dashData.recientes?.length > 0 && (
                <div style={{ padding: "1rem", borderRadius: "4px", background: COLORS.card, border: `1px solid ${COLORS.border}`, marginBottom: "1rem" }}>
                  ...
                  {dashData.recientes.map((p, i) => (
                    <div key={i} style={{ display: "flex", alignItems: "center", gap: "0.5rem", padding: "0.4rem 0", borderBottom: i < dashData.recientes.length - 1 ? `1px solid ${COLORS.border}` : "none" }}>
                      <span style={{ padding: "2px 6px", borderRadius: "4px", fontSize: "0.7rem", fontWeight: 700, background: p.viable ? "rgba(0,184,148,0.15)" : "rgba(225,112,85,0.15)", color: p.viable ? COLORS.green : COLORS.red }}>{p.viable ? "VIABLE" : "NO"}</span>
              ...
              {dashData.leads?.length > 0 && (
                <div style={{ padding: "1rem", borderRadius: "4px", background: COLORS.card, border: `1px solid ${COLORS.border}` }}>
                  <div style={{ fontWeight: 700, color: COLORS.text, fontSize: "0.9rem", marginBottom: "0.75rem" }}>
                  ...
                  {dashData.leads.map((l, i) => (
                    <div key={i} style={{ display: "flex", alignItems: "center", gap: "0.5rem", padding: "0.4rem 0", borderBottom: i < dashData.leads.length - 1 ? `1px solid ${COLORS.border}` : "none" }}>
                      ...
                      <span style={{ marginLeft: "auto", padding: "1px 6px", borderRadius: "4px", fontSize: "0.68rem", fontWeight: 600, background: l.estado === "nuevo" ? "rgba(243,156,18,0.15)" : "rgba(0,184,148,0.15)", color: l.estado === "nuevo" ? COLORS.orange : COLORS.green }}>{l.estado}</span>
                    </div>
                  ))}
                </div>
              )}
      ...
      {view === "config" && (
        <div style={{ maxWidth: "520px", margin: "0 auto", padding: "1.5rem 1rem" }}>
          <div style={{ background: COLORS.card, borderRadius: "4px", padding: "1.5rem", border: `1px solid ${COLORS.border}` }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
              <div style={{ padding: "0.75rem", borderRadius: "4px", background: "rgba(0,184,148,0.06)", border: "1px solid rgba(0,184,148,0.15)" }}>
            ...
            <button data-testid="config-save-tasas" onClick={saveTasas} disabled={tasasSaving}
              style={{ marginTop: "1.25rem", width: "100%", padding: "0.75rem", borderRadius: "4px", border: "none", background: `linear-gradient(135deg, ${COLORS.accent}, #4834d4)`, color: "#fff", fontWeight: 700, fontSize: "0.95rem", cursor: tasasSaving ? "wait" : "pointer" }}>
              <i className={`fa ${tasasSaving ? "fa-spinner fa-spin" : "fa-save"}`} style={{ marginRight: "0.5rem" }}></i>
              {tasasSaving ? "Guardando..." : "Guardar Tasas"}
            </button>

            <div style={{ marginTop: "1rem", padding: "0.75rem", borderRadius: "4px", background: "rgba(212,175,55,0.08)", border: `1px solid rgba(212,175,55,0.2)` }}>
      ...
      {view === "predictor" && <div style={{ maxWidth: "520px", margin: "0 auto", padding: "1.5rem 1rem" }}>
        ...
        <div data-testid="predic-mode-selector" style={{ display: "flex", gap: "0.5rem", marginBottom: "1.5rem" }}>
          {[["subsidio", "Con Subsidio"], ["sin_subsidio", "Sin Subsidio"]].map(([key, label]) => (
            <button key={key} data-testid={`predic-mode-${key}`} onClick={() => { setModo(key); setResult(null); }}
              style={{
                flex: 1, padding: "0.75rem", borderRadius: "4px", border: `2px solid ${modo === key ? COLORS.accent : COLORS.border}`,
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "1rem", marginBottom: "1.25rem" }}>
          ...
          {/* Section: Cliente */}
          <div style={{ padding: "0.75rem", borderRadius: "4px", background: "rgba(108,92,231,0.04)", border: `1px solid ${COLORS.border}` }}>
          ...
          {/* Section: Propiedad */}
          <div style={{ padding: "0.75rem", borderRadius: "4px", background: "rgba(212,175,55,0.04)", border: `1px solid ${COLORS.border}` }}>
            ...
            {parseFloat(form.valor_propiedad) > 0 && parseFloat(form.monto_credito) > 0 && (
              <div style={{ marginTop: "0.4rem", display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
                <span style={{ padding: "2px 6px", borderRadius: "4px", fontSize: "0.65rem", fontWeight: 600,
                ...
                {parseFloat(form.pie) > 0 && (
                  <span style={{ padding: "2px 6px", borderRadius: "4px", fontSize: "0.65rem", fontWeight: 600, background: "rgba(108,92,231,0.1)", color: COLORS.accentLight }}>
          ...
          {/* Section: Ingresos */}
          <div style={{ padding: "0.75rem", borderRadius: "4px", background: "rgba(0,184,148,0.04)", border: `1px solid ${COLORS.border}` }}>
            ...
            {(parseCLP(form.renta_fija) > 0 || parseCLP(form.renta_variable) > 0 || parseCLP(form.renta_honorarios) > 0) && (() => {
              ...
              return (
                <div style={{ marginTop: "0.4rem", display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
                  <span data-testid="predic-renta-total" style={{ padding: "2px 6px", borderRadius: "4px", fontSize: "0.65rem", fontWeight: 600, background: "rgba(0,184,148,0.12)", color: COLORS.green }}>
                    Total: ${total.toLocaleString("es-CL")}
                  </span>
                  {castigo > 0 && (
                    <span data-testid="predic-renta-castigo" style={{ padding: "2px 6px", borderRadius: "4px", fontSize: "0.65rem", fontWeight: 600, background: "rgba(243,156,18,0.12)", color: COLORS.orange }}>
        ...
        <button data-testid="predic-advanced-toggle" onClick={() => setShowAdv(!showAdv)}
          style={{ width: "100%", padding: "0.6rem", borderRadius: "4px", border: `1px solid ${COLORS.border}`, background: showAdv ? "rgba(108,92,231,0.1)" : "transparent", color: COLORS.textMuted, cursor: "pointer", fontSize: "0.82rem", fontWeight: 600, marginBottom: "0.75rem", display: "flex", alignItems: "center", justifyContent: "center", gap: "0.4rem" }}>
          <i className={`fa fa-${showAdv ? "chevron-up" : "chevron-down"}`}></i>
          Evaluacion Avanzada
        </button>

        {showAdv && (
          <div data-testid="predic-advanced-section" style={{ padding: "1rem", borderRadius: "4px", background: "rgba(108,92,231,0.06)", border: `1px solid ${COLORS.border}`, marginBottom: "0.75rem" }}>
        ...
        <button data-testid="predic-calculate-btn" onClick={handlePredict} disabled={loading}
          style={{
            width: "100%", padding: "1rem", borderRadius: "4px", border: "none", fontSize: "1.1rem", fontWeight: 800, cursor: loading ? "wait" : "pointer",
        ...
        {result && !result.error && (
          <button data-testid="predic-comparar-btn" onClick={handleComparar} disabled={comparandoLoad}
            style={{ width: "100%", marginTop: "1rem", padding: "0.85rem", borderRadius: "4px", border: "2px solid rgba(212,175,55,0.4)", background: "linear-gradient(135deg, rgba(212,175,55,0.15), rgba(212,175,55,0.05))", color: COLORS.gold, fontWeight: 700, fontSize: "0.95rem", cursor: "pointer", transition: "all 0.3s" }}>
        ...
        {/* Comparison Results */}
        {comparacion && (
          <div data-testid="predic-comparacion" style={{ marginTop: "1rem", borderRadius: "4px", overflow: "hidden", border: `2px solid rgba(212,175,55,0.3)`, background: COLORS.card }}>
            ...
            <div style={{ padding: "0.75rem 1.25rem", display: "flex", gap: "0.5rem", flexWrap: "wrap", borderBottom: `1px solid ${COLORS.border}` }}>
              {comparacion.datos_comparacion && (
                <span style={{ padding: "4px 10px", borderRadius: "4px", fontSize: "0.75rem", fontWeight: 600, background: "rgba(212,175,55,0.2)", color: COLORS.text }}>
              ...
              <span style={{ padding: "4px 10px", borderRadius: "4px", fontSize: "0.75rem", fontWeight: 700, background: "rgba(212,175,55,0.15)", color: COLORS.gold }}>
                Tu tasa: {comparacion.resumen?.tasa_mutuaria}%
              </span>
              <span style={{ padding: "4px 10px", borderRadius: "4px", fontSize: "0.75rem", fontWeight: 600, background: "rgba(108,92,231,0.12)", color: COLORS.accentLight }}>
                Promedio bancos: {comparacion.resumen?.tasa_promedio_bancos}%
              </span>
              {comparacion.resumen?.diferencia_dividendo_mensual > 0 && (
                <span style={{ padding: "4px 10px", borderRadius: "4px", fontSize: "0.75rem", fontWeight: 600, background: "rgba(0,184,148,0.12)", color: COLORS.green }}>
            ...
            <div style={{ padding: "0.75rem 1.25rem", background: "rgba(0,184,148,0.04)", borderTop: `1px solid ${COLORS.border}` }}>
              ...
              <button data-testid="predic-comparar-pdf" onClick={async () => {
                ...
                style={{ width: "100%", marginTop: "0.75rem", padding: "0.7rem", borderRadius: "4px", border: "none", background: "linear-gradient(135deg, rgba(212,175,55,0.9), rgba(180,140,40,0.9))", color: "#fff", fontWeight: 700, fontSize: "0.85rem", cursor: "pointer" }}>
        ...
        {result && !result.error && (
          <div data-testid="predic-contact-section" style={{ marginTop: "1.25rem", borderRadius: "4px", padding: "1.25rem", border: `1px solid ${COLORS.border}`, background: COLORS.card }}>
            {contactSent ? (
              ...
              <>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "0.6rem" }}>
                  ...
                  <button data-testid="predic-contact-btn" onClick={handleContact}
                    style={{ padding: "0.8rem", borderRadius: "4px", border: "none", background: `linear-gradient(135deg, ${COLORS.green}, #00a884)`, color: "#fff", fontWeight: 700, fontSize: "0.95rem", cursor: "pointer", boxShadow: `0 4px 15px rgba(0,184,148,0.3)` }}>
        ...
        <div style={{ marginTop: "1.5rem" }}>
          <button data-testid="predic-calc-toggle" onClick={() => setShowCalc(!showCalc)}
            style={{ width: "100%", padding: "0.65rem", borderRadius: "4px", border: `1px solid ${COLORS.border}`, background: "transparent", color: COLORS.accentLight, cursor: "pointer", fontSize: "0.85rem", fontWeight: 600 }}>
            <i className={`fa fa-calculator`}></i> Calculadora de Endeudamiento {showCalc ? "▲" : "▼"}
          </button>
          {showCalc && (
            <div data-testid="predic-debt-calculator" style={{ marginTop: "0.75rem", padding: "1rem", borderRadius: "4px", border: `1px solid ${COLORS.border}`, background: COLORS.card }}>
              <div style={{ display: "flex", flexDirection: "column", gap: "0.6rem" }}>
                ...
                <button data-testid="predic-calc-btn" onClick={handleCalc}
                  style={{ padding: "0.6rem", borderRadius: "4px", border: "none", background: `linear-gradient(135deg, ${COLORS.orange}, #e67e22)`, color: "#fff", fontWeight: 700, cursor: "pointer" }}>
                  CALCULAR CUOTA
                </button>
                {calcResult && (
                  <div data-testid="predic-calc-result" style={{ padding: "0.75rem", borderRadius: "4px", background: "rgba(243,156,18,0.1)", border: `1px solid rgba(243,156,18,0.3)`, marginTop: "0.3rem" }}>
      ...
      {showChat && chatEnabled && (
        <div data-testid="predic-chat-panel" style={{ position: "fixed", bottom: "5rem", right: "1.5rem", width: "360px", maxHeight: "500px", borderRadius: "4px", background: COLORS.card, border: `2px solid ${COLORS.accent}`, boxShadow: "0 8px 40px rgba(0,0,0,0.4)", zIndex: 1000, display: "flex", flexDirection: "column", overflow: "hidden" }}>
          {/* Chat Header */}
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: "0.75rem", display: "flex", flexDirection: "column", gap: "0.5rem", maxHeight: "340px", minHeight: "200px" }}>
            ...
            {chatLoading && (
              <div style={{ display: "flex", justifyContent: "flex-start" }}>
                <div style={{ padding: "0.5rem 0.75rem", borderRadius: "4px", background: "rgba(255,255,255,0.05)", border: `1px solid ${COLORS.border}`, fontSize: "0.8rem", color: COLORS.textMuted }}>
          ...
          <div style={{ padding: "0.5rem", borderTop: `1px solid ${COLORS.border}`, display: "flex", gap: "0.4rem" }}>
            <input data-testid="predic-chat-input" value={chatInput} onChange={e => setChatInput(e.target.value)}
              ...
              style={{ flex: 1, padding: "0.5rem 0.75rem", borderRadius: "4px", border: `1px solid ${COLORS.border}`, background: "rgba(255,255,255,0.05)", color: COLORS.text, fontSize: "0.82rem", outline: "none" }} />
            <button data-testid="predic-chat-send" onClick={sendChat} disabled={chatLoading || !chatInput.trim()}
              style={{ padding: "0.5rem 0.75rem", borderRadius: "4px", border: "none", background: COLORS.accent, color: "#fff", cursor: "pointer", fontSize: "0.85rem" }}>
