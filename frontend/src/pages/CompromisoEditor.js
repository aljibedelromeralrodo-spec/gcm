...
<p style="margin:0;color:#000000"><b>DÉCIMO: PRECIO Y FORMA DE PAGO.</b> El precio de la venta es la suma de ${ufTxt(totalUF)}. De este monto, el comprador paga en este acto la suma de ${ufTxt(pieUF)}, equivalentes a ${clpTxt(pieUF, uf)} al valor UF del día de hoy (${uf > 0 ? fmtCLP(uf) : "[POR DEFINIR]"} al ${new Date().toLocaleDateString("es-CL").replace(/-/g, "/")}), dinero que el vendedor declara recibir a su entera y total satisfacción, otorgando por este instrumento el más amplio y completo finiquito respecto de dicha suma.</p>
</div>` : `
...
<p>El Vendedor declara bajo juramento haber recibido del Comprador, de manera íntegra, total y oportuna, la suma de ${ufTxt(pieUF)}, equivalente a ${clpTxt(pieUF, uf)} al valor UF del día de hoy (${uf > 0 ? fmtCLP(uf) : "[POR DEFINIR]"} al ${new Date().toLocaleDateString("es-CL").replace(/-/g, "/")}), por concepto de pie del precio de la compraventa. En consecuencia, el Vendedor otorga al Comprador el más amplio, completo y total finiquito respecto de dicha suma, declarándola íntegramente pagada y renunciando expresamente a toda acción, cobro o reclamación posterior derivada de su pago.</p>` : "";
  return `
