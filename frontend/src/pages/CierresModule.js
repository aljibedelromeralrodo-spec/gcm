...
export default function CierresModule() {
  ...
  const inp = { width: "100%", padding: "0.45rem", borderRadius: 6, border: "1px solid rgba(148,163,184,0.3)", background: "#1e293b", color: "#e2e8f0", fontSize: 13 };
    ...
    <div data-testid="cierres-module" style={{ display: "grid", gap: "1rem" }}>
      <div style={{ background: "rgba(15,23,42,0.85)", border: "1px solid rgba(148,163,184,0.15)", borderRadius: 12, padding: "1.2rem 1.4rem" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
          ...
          <button onClick={load} data-testid="cierres-refresh" style={{ background: "rgba(148,163,184,0.1)", border: "1px solid rgba(148,163,184,0.3)", color: "#94a3b8", borderRadius: 8, padding: "0.4rem 0.9rem", fontSize: 12.5, cursor: "pointer" }}>
      ...
      {loading ? (
        ...
        <div key={grupo} data-testid="cierres-grupo" style={{ background: "rgba(15,23,42,0.85)", border: "1px solid rgba(148,163,184,0.15)", borderRadius: 12, padding: "1rem 1.2rem" }}>
          ...
          <div style={{ display: "grid", gap: 8 }}>
            {lista.map(r => (
              <div key={r.id} data-testid={`cierre-row-${r.id}`} style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap", background: r.toca_preguntar ? "rgba(245,158,11,0.06)" : "rgba(30,41,59,0.6)", border: r.toca_preguntar ? "1.5px solid rgba(245,158,11,0.4)" : "1px solid rgba(148,163,184,0.12)", borderRadius: 10, padding: "0.7rem 1rem" }}>
                ...
                <div style={{ display: "flex", gap: 8, marginLeft: "auto" }}>
                  <button data-testid={`cierre-edit-${r.id}`} onClick={() => setEdit({ ...r })}
                    style={{ background: "rgba(148,163,184,0.1)", border: "1px solid rgba(148,163,184,0.3)", color: "#94a3b8", borderRadius: 8, padding: "0.45rem 0.9rem", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>
                  ...
                  <button data-testid={`cierre-consultar-${r.id}`} onClick={() => consultar(r)} disabled={sending === r.id}
                    ...
                    style={{ background: r.toca_preguntar ? "#0d9488" : "rgba(13,148,136,0.25)", border: "none", color: "#fff", borderRadius: 8, padding: "0.45rem 1rem", fontSize: 12, fontWeight: 800, cursor: "pointer" }}>
                  ...
                  <button data-testid={`cierre-no-continua-${r.id}`} onClick={() => marcarNoContinua(r)}
                    ...
                    style={{ background: "rgba(185,28,28,0.15)", border: "1.5px solid #b91c1c", color: "#f87171", borderRadius: 8, padding: "0.45rem 0.9rem", fontSize: 12, fontWeight: 800, cursor: "pointer" }}>
